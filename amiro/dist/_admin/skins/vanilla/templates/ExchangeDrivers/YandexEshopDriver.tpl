%%include_language "templates/ExchangeDrivers/lang/YandexEshopDriver.lng"%%

<!--#set var="import_form" value="
<script name="javascript">

function driverFunc_YandexEshopDriver_import(action) {
    if (action == 'on') {
        yandex_to_dest_category();
    } else {
        document.forms[_cms_document_form].cat_id.disabled  = false;
    }
}

function yandex_to_dest_category() {
    if (document.forms[_cms_document_form].import_create_cat_arr.value == 'yandex_import_default') {
        document.forms[_cms_document_form].cat_id.disabled  = false;
    }
    else {
...
<tr name="##driver_name##_##driver_type##" style="display:##display##;">
  ...
  <td>
  <select name="import_create_cat_arr" OnChange="yandex_to_dest_category();">
...
<tr name="##driver_name##_##driver_type##" style="display:##display##;">
  ...
  <td colspan=2>
    <select name="import_img_type" OnChange="show_image_get(this.value);">
...
<script name="javascript">
function set_ym_export_filename() {
    if (document.forms[_cms_document_form].export_type.value == 'Yandex.Direct.CSV') {
        document.forms[_cms_document_form].export_save_filename.value = 'yandex_export.csv';
    } else {
        document.forms[_cms_document_form].export_save_filename.value = 'yandex_export.xml';
    }
}
</script>
...
<tr name="##driver_name##_##driver_type##" style="display:##display##;">
  ...
  <td colspan=2>
    ...
    %%export_type%%
    <select name="export_type" onChange="set_ym_export_filename();">
      ##export_type_rows##
    </select>
    ##else##
