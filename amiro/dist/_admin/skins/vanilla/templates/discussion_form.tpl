...
<!--#set var="form_preview_btn" value="<input type="submit" class="but-244" value="%%bb_preview%%" name="preview" onClick="return txtEd.previewButtonOnClick(this)" />"-->
...
##endif##
##if(answer_for)##
function OnObjectChanged_discussion_form(name, first_change, evt)
{
    if (name == 'realAnswer') {
        var oForm = document.forms[_cms_document_form];
        oForm.id_parent.value = oForm.realAnswer.checked ? ##_parent_id## : ##_root_id##;
    }
    return true;
}
addFormChangedHandler(OnObjectChanged_discussion_form);
##endif##
...
form.elements['action_original'].value = 'apply';
##endif##
    return true;
}
...
##-- <input type="hidden" name="id_ext_module" value="##id_ext_module##" /> --##
##if(is_reply)##<input type="hidden" name="is_reply" value="1" />##endif##
##-- <input type="hidden" name="ext_module" value="##ext_module##" /> --##
...
<table cellspacing="5" cellpadding="0" border="0" width="100%" class="frm">
	...
	<col width="*">
<tr>
    <td colspan="2"><input type="checkbox" name="public" id="public" ##public## value="1"> <label for="public">%%public%%</label></td>
</tr>
<tr><td>%%module%%:</td><td>##ext_module_caption##</td></tr>
<tr>
    <td>%%date%%*:</td>
    <td><input type="text" name="date" class="field fieldDateTime" value="##fdate##"  disabled></td>
</tr>
<tr>
    <td>%%author%%:</td>
