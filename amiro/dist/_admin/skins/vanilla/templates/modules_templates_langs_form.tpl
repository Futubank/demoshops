...
var _cms_document_form = 'modulestemplateslangsform';
...
function OnObjectChanged_modules_templates_langs_form(name, first_change, evt){
  ##pictures_js_script##
  return true;
}
addFormChangedHandler(OnObjectChanged_modules_templates_langs_form);
...
function LocalBodyOnLoad() {
    setCaretPos("txtArea", document.modulestemplateslangsform.ta_position.value);
    if(document.modulestemplateslangsform.ta_highlight.value != 'no'){
        oTextEditor.highlight(true);
        document.modulestemplateslangsform.ta_highlight_flag.checked = true;
    }else{
        document.modulestemplateslangsform.ta_highlight_flag.checked = false;
    }
}
...
//-->
  ...
  <br>
    <form action=##script_link## method=post enctype="multipart/form-data" name="modulestemplateslangsform" onSubmit="return CheckForm(window.document.modulestemplateslangsform);">
	...
	<col width="*">
     <tr>
       ...
       <td>
         ...
         ##name##.lng
         ##else##
         <input type=text name=name class="field field50" value="##name##" maxlength="255"><b>.lng</b>
         ##endif##
       </td>
     </tr>
     <tr>
     ...
     <tr>
       <td colspan=2>
         ...
         <div style="overflow: hidden">
             %%content%%:
                          <label style="display: block; float: right; padding-bottom:6px;"><input type="checkbox" name="ta_highlight_flag" value="1" onChange="oTextEditor.highlight(this.checked); document.modulestemplateslangsform.ta_highlight.value = this.checked ? 'yes' : 'no'; AMI.find('#_hl_search_bar').style.display = this.checked ? 'block' : 'none';"> %%highlight_code%%</label>
