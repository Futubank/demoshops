 ...
 * @version    $Id$ 
 * @since      5.10.0 
 * @size       96750 xkqwsklwixzxwxilpuqlylmrwrguklxqsulrzplyxuuqmxtzgrpgukmtqxqylzxpsknzpnir
 */
?><?php
 ...
 *
 * @event      on_rate ext_rating $modId ('ext_rating' everytime)
...
/**
 * Allows to customize SQL query to select products during data exchange.
 *
 * function cstHandleDataExchangeSQL($name, array $aEvent, $handlerModId, $srcModId){<br />
 * &nbsp;&nbsp;&nbsp;&nbsp;$pfx = $aEvent['aSQL]['default_prefix'];<br />
 * &nbsp;&nbsp;&nbsp;&nbsp;// select extra field `id_owner`<br />
 * &nbsp;&nbsp;&nbsp;&nbsp;$aEvent['aSQL]['select'] .= ', ' . $pfx . 'id_owner';<br />
 * &nbsp;&nbsp;&nbsp;&nbsp;// select products having `price` > 1000 only<br />
 ...
 *
 * @event      v5_on_dataexchange_sql AMI_Event::MOD_ANY
 * @eventparam string type    Data exchange type: 'import' / 'export'
 * @eventparam string driver  Data exchange driver name
 * @eventparam string modId   Module id
 * @eventparam &array aSQL    Array of SQL query parts
 * @since      7.0.0
 */
...
/**
 * Fires before action will be processed in old environment.
 *
 * Allows to modify action, array of item Ids for group action.<br />
 * Set $aEvent['_discard'] to TRUE to discard action.
 *
 * @event      v5_on_action_verification AMI_Event::MOD_ANY
 * @eventparam string  modId    Module Id
 * @eventparam &string action   Action
 * @eventparam int     id       Item Id
 * @eventparam &array  aGrpIds  Array of item Ids for group action
 * @eventparam bool   _discard  Set to TRUE to discard action
 * @since      7.0.0
 */

/**
 * Fires after old environment extensions processed action.
 *
 * Other event parameters depend on action.<br />
 * <a href="__filesource/fsource_Service__exampleseventsv5_events.php.html">Usage example</a>.
 *
 * @event      v5_on_ext_action AMI_Event::MOD_ANY
 * @eventparam string        modId          Module Id
 * @eventparam string        action         Action
 * @eventparam CMS_ActModule oOldEnvModule  Old environment module object
 * @since      7.0.0
 */

/**
 * Fires before admin page will be combined from several parts.
 *
 * Other event parameters depend on module.<br />
 * Allows to modify form or other part HTML code.<br />
 * <a href="__filesource/fsource_Service__exampleseventsv5_events.php.html">Usage example</a>.
 *
 * @event      v5_on_render_page_adm AMI_Event::MOD_ANY
 * @eventparam string  modId  Module Id
 * @since      7.0.0
 */

/**
 * Fires before initializing of front filter.
 *
 * @event      v5_on_front_filter_init $modId
 * @eventparam bool discard_ext_init_front_filter  Flag to discard initializing of extensions
 * @since      7.0.0
 */

/**
 * Allows to modify old environment SQL filter.
 *
 * @event      v5_on_apply_filters $modId
 * @eventparam string        &sqlPart   Raw SQL filter part
 * @eventparam string        sqlPrefix  Filter field prefix (i. e. 'i.' for items, 'c.' for categories etc.)
 * @eventparam CMS_ActModule oEngine    Old environment module object
 * @since      7.0.0
 */

/**
 * Allows to modify old environment SQL filter for admin list.
 *
 * @event      v5_on_admin_list_view AMI_Event::MOD_ANY
 * @eventparam string        modId     Module Id
 * @eventparam CMS_ActModule oEngine   Old environment module object
 * @eventparam Browser       oBrowser  Old environment browser object
 * @since      7.0.0
 */

/**
 * Allows to discard data exchange.
 *
 * @event      v5_on_dataexchange_start AMI_Event::MOD_ANY
 * @eventparam string                 modId     Module Id
 * @eventparam CMS_DataExchangeModule oEngine   Old environment module object
 * @eventparam bool                   _discard  Set to TRUE to discard action
 * @since      7.0.0
 */

/**
 * Allows to discard to import/export product or modify product fields during data exchange.
 *
 * @event      v5_on_dataexchange_item AMI_Event::MOD_ANY
 * @eventparam string type     Data exchange type: 'import' / 'export'
 * @eventparam string driver   Data exchange driver name
 * @eventparam string modId    Module Id
 * @eventparam array  &aItem   Product data
 * @eventparam bool  _discard  Set to TRUE to discard action
 * @since      7.0.0
 */
  ...
  * Allows to discard to import/export product or modify product fields during data exchange.
  *
  * @event      v5_on_dataexchange_download_image AMI_Event::MOD_ANY
  * @eventparam string type         Data exchange type: 'import' / 'export'
  * @eventparam string driver       Data exchange driver name
  * @eventparam string modId        Module Id
  * @eventparam string &sourceURL   Source image URL
  * @eventparam string &targetName  Target file name
  * @since      7.0.0
  */

/**
 * Fires on building item search index.
 *
 * Example:
 * <code>
 * // "_local/common_functions.php"
 * // Allowing to search "/" symbol
 *
 * AMI_Event::addHandler('on_search_index', 'cstOnSearchIndex', 'eshop_item');
 * AMI_Event::addHandler('on_search_command', 'cstOnSearchCommand', 'eshop_item');
 *
 ...
 * }
 *
 * function cstOnSearchCommand($name, array $aEvent, $handlerModId, $srcModId){
 *     $aEvent['oCommand']->Allowed = "_\*a-zA-ZёЁа-яА-Я0-9.\-\/";
 *
 *     return $aEvent;
 * }
 * </code>
 *
 * @event      on_search_index $modId
 * @eventparam string sourceContent  Source item content6
 * @eventparam string &content       Prepared item content
 * @eventparam array &aReplacement   Array of replacements for preg_rep[lace
 * @since      7.0.0
 */

/**
 * Fires on building search query.
 *
 * @event      on_search_command $modId
 * @eventparam searchCmd oCommand  Search command
 * @since      7.0.0
 */

/**
 * Fires on creating e-shop order payment button.
 *
 * @event v5_on_eshop_order_before_pay_button_params AMI_Event::MOD_ANY
 * @eventparam Array aData  Array of payment data
 * @since      7.0.0
 */

/**
 * Fires on building list query.
 *
 * Arguments will be described later.
 *
 * @event v5_on_get_list_sql AMI_Event::MOD_ANY
 * @eventparam string sql_fields
 * @eventparam array  sql_from_tables
 * @eventparam string sql_from
 * @eventparam string order_field
 * @eventparam string order_addon
 * @eventparam int    options
 * @eventparam array  order_replace
 * @eventparam string default_prefix
 * @eventparam array  joined_tables
 * @eventparam string order_col
 * @eventparam string order_col_pref
 * @eventparam string force_order
 * @eventparam string force_limit
 * @eventparam string group_by
 * @eventparam array  calculating
 * @eventparam array  calc_options
 * @eventparam array  grouping
 * @eventparam string order_sql
 * @since      7.0.0
 */
...
final class AMI{
    ...
    */
    ...
    public static function validateModId($modId){
        // @todo: remove user/rating workaround after 7.0.2
        return (bool)(preg_match('/^[a-z](?:[a-z\d]|_[a-z])+$/', $modId) || preg_match('/^plugin\_(\d+)$/', $modId) || ('users/rating' === $modId));
    }
     ...
     * @since  7.0.0
     */
    public static function getOptionLazy($modId, $name = ''){
        if(isset($GLOBALS['Core'])){
            return
                $modId !== 'core' // || ($GLOBALS['Core'] instanceof AMI_iCore)
                ? $GLOBALS['Core']->getModOptionLazy($modId, $name)
                : $GLOBALS['Core']->getOptionLazy($name);
        }
        trigger_error("Unknown option '" . $name . "' in module '" . $modId . "'", E_USER_WARNING);
    }
     ...
     * @since  7.0.0
     */
    public static function issetOptionLazy($modId, $name){
        if(isset($GLOBALS['Core'])){
            return
                $modId !== 'core' || ($GLOBALS['Core'] instanceof AMI_iCore)
                ? $GLOBALS['Core']->issetModOptionLazy($modId, $name)
                : $GLOBALS['Core']->issetOptionLazy($name);
        }
     ...
     * @since  7.0.0
     */
    public static function issetAndTrueOptionLazy($modId, $name){
        return self::issetOptionLazy($modId, $name) && self::getOptionLazy($modId, $name);
    }
     ...
     * @since  5.12.0
     */
    public static function setOption($modId, $name, $value){
        if(isset($GLOBALS['Core'])){
            if($modId != 'core'){
     ...
     * @since  7.0.0
     */
    public static function setOptionLazy($modId, $name, $value){
        if(isset($GLOBALS['Core'])){
            if($modId != 'core'){
                $GLOBALS['Core']->setModOptionLazy($modId, $name, $value);
            }else{
                $GLOBALS['Core']->setOptionLazy($name, $value);
            }
        }
    }
     ...
     * @since  6.0.2
     */
    public static function replaceOptions($modId, array $aOptions){
        $aSavedOptions = array();
     ...
     * @amidev temporary
     */
    public static function setProperty($modId, $name, $value){
        if(isset($GLOBALS['Core'])){
            if($modId != 'core'){
     ...
     * @since  6.0.2
     */
    public static function saveOptions($modId){
        global $Core;
        ...
        if(isset($Core) && ($Core instanceof CMS_Core)){
            trigger_error("AMI::saveOptions('" . $modId . "') is called");
     ...
     * @see    options.php
     */
    public static function getPluginOption($pluginId, $name, $secret = ''){
     ...
     * @return mixed
     */
    public static function getVersion($product = false, $type = false){
        global $Core;
    ...
    public static function isResource($resId, $autoload = TRUE){
        ...
        $result = isset(self::$aResMapping[$resId]);

        if($result){
            self::cleanupResourceMapping($resId);
        }
    ...
    public static function getResource($resId, array $aArgs = array(), $isLazy = false){
        ...
        if($isLazy){
            // Retutn lazy facade object for later real object creation
            return new AMI_FacadeLazy($resId, $aArgs);
        }
        if(empty(self::$aResMapping[$resId])){
            throw new AMI_Exception(
                "Unknown resource '" . $resId . "'",
    ...
    */
    ...
    public static function toolbarEnabled(){
        return AMI::issetAndTrueOption('core', 'allow_edit_at_front') && AMI_Registry::get('ami/editMode/displayToolbar', FALSE);
    }
    ...
    public static function editModeEnabled(){
        return AMI::issetAndTrueOption('core', 'allow_edit_at_front') && AMI_Registry::get('ami/editMode/enabled', FALSE);
    }

    /**
     * Returns roles and access rights for passed module and item owner.
     *
     * @param  string $modId    Module Id
     * @param  int    $ownerId  Record owner Id (0 for authorized user)
     * @param  string $mode     Mode
     * @return array
     * @amidev Temporary?
     */
    public static function getAccessRights($modId, $ownerId = 0, $mode = 'edit_at_front'){
        $aUser = AMI::getSingleton('env/session')->getUserData(TRUE);
        if($aUser){
            do{
                if('edit_at_front' == $mode && empty($aUser['ami_efa'])){
                    break;
                }
                $aRights = $aUser['ami_ar'];
                if($aRights['system_user']){
                    return array(
                        'roles'   => array('system_user'),
                        'actions' => array('edit')
                    );
                }
                if(0 === $ownerId){
                ...
                if(
                    isset($aRights['rights'][$modId]) &&
                    (
                        $ownerId == $aUser['id'] || in_array('moderator', $aRights['rights'][$modId]['roles'])
                    )
                ){
                    return $aRights['rights'][$modId];
                }
            }while(FALSE);
    ...
    /**
     * Returns TRUE if passed owner has passwed actions.
     *
     * @param  string $modId     Module Id
     * @param  int    $ownerId   Record owner Id (0 for authorized user)
     ...
     * @amidev Temporary?
     */
    public static function checkAccessRights($modId, $ownerId, array $aActions, $mode = 'edit_at_front'){
        $aRights = self::getAccessRights($modId, $ownerId, $mode);

        return
            array_intersect($aRights['actions'], $aActions) == $aActions;
    }

    /**
     * Returns TRUE if common front elements filterring is needed.
     *
     * @param  string $modId     Module Id
     * @return bool
     * @amidev Temporary?
     */
    public static function doFilterFrn($modId){
        if(AMI::editModeEnabled()){
            $aRights = self::getAccessRights($modId);
            foreach($aRights['roles'] as $role){
                if(in_array($role, array('system_user', 'moderator'))){
                    return FALSE;
                }
            }
        }
    ...
    private static function _getExpectedClassNameByResourceId($resId, $autoload = TRUE){
        ...
        if(AMI_ModDeclarator::getInstance()->isRegistered($modId)){
            ...
            // #CMS-11790
            if(defined('AMI_ALLOW_LOCAL_MODULES')){
        ...
        $aDefaultResourceMappingSchema = array(
            ...
            $modId . '/module/controller/skin' => $classPrefix . '_Skin',
            ...
            $modId . '/view/skin' => $classPrefix . '_ViewSkin',

            // service
            $modId . '/service' => $classPrefix . '_Service',

            // trusted service
            $modId . '/trusted_service' => $classPrefix . '_TrustedService',
        ...
        if(isset($aDefaultResourceMappingSchema[$resId])){
            $class = $aDefaultResourceMappingSchema[$resId];
            self::$aClassToModId[$class] = $modId;
            if(class_exists($class, FALSE)){
                $parent = get_parent_class($class);
                if(preg_match('/^(Hyper|AMI)_/', $parent)){
                    AMI_Registry::set('AMI/Environment/Core/skipResourceMapping', $class);
                }
            }
        }
    ...
    /**
     * Cleanes up resource mapping for configurations / hypermodules classes.
     *
     * @param  string $resId  Resource Id
     * @return void
     */
    private static function cleanupResourceMapping($resId){
        // #CMS-11861
        $classToCleanup = AMI_Registry::get('AMI/Environment/Core/skipResourceMapping', FALSE);
        if($classToCleanup == self::$aResMapping[$resId]){
            unset(self::$aResMapping[$resId]);
            AMI_Registry::delete('AMI/Environment/Core/skipResourceMapping');
        }
    }
...
class AMI_Skin {
    ...
    /**
     * Special module mode setted for skin
     *
     * @var array
     */
    protected static $aModSkinMode = array();

    /**
     * Initializes "Skin" mode.
     *
     * @amidev Temporary
     */
    ...
    /**
     * Returns true if skin environment is active.
     *
     * @return bool
     */
    public static function skinEnvActive(){
        return AMI::issetAndTrueOption('core', 'allow_edit_at_front') && self::$isActive;
    }

    /**
     * Setup special mode for module to run with skinned components.
     *
     * @param string $modId  Module Id
     * @amidev Temporary
     */
    public static function setupModuleSpecialMode($modId){
        AMI::setProperty($modId, 'front_request_types', array('ajax'));
        self::$aModSkinMode[$modId] = TRUE;
    }

    /**
     * Checks for special skin mode for module.
     *
     * @param string $modId  Module Id
     * @return bool
     * @amidev Temporary
     */
    public static function isModuleSpecialMode($modId){
        return self::$isActive && isset(self::$aModSkinMode[$modId]) && self::$aModSkinMode[$modId];
    }
     ...
     * @param  array   $aSkin  Array of skin data
     * @return void
     * @amidev Temporary
     */
    public static function add($name, $aSkin = FALSE){
        if($aSkin === FALSE){
            // Defaults
            $aSkin =
                array(
                    'path' => array(
                        // Stylesheets
                        'css/main.css'    => '~base/css/main.css',
                        'css/module.css'  => '~base/css/module.css',
                        'css/plugins.css' => '~base/css/plugins.css',
                        'css/skin.css'    => '~base/css/skin.css',
                        'css/toolbar.css' => '~base/css/toolbar.css',
                        // Javascript
                        'js/app.js'       => '~base/js/app.js',
                        'js/init.js'      => '~base/js/init.js',
                        'js/plugins.js'   => '~base/js/plugins.js',
                        'js/skin.js'      => '~base/js/skin.js',
                        'js/skin_init.js' => '~base/js/skin_init.js',
                        'js/stats.js'     => '~base/js/stats.js',
                        // Templates
                        'templates/_form.tpl'               => '~base/templates/_form.tpl',
                        'templates/eshop_cat_form.tpl'      => '~base/templates/eshop_cat_form.tpl',
                        'templates/eshop_item_form.tpl'     => '~base/templates/eshop_item_form.tpl',
                        'templates/eshop_order_form.tpl'    => '~base/templates/eshop_order_form.tpl',
                        'templates/eshop_order_list.tpl'    => '~base/templates/eshop_order_list.tpl',
                        'templates/ext_image.tpl'           => '~base/templates/ext_image.tpl',
                        'templates/list_icons.tpl'          => '~base/templates/list_icons.tpl',
                        'templates/mod.tpl'                 => '~base/templates/mod.tpl',
                        'templates/options_popup.tpl'       => '~base/templates/options_popup.tpl',
                        'templates/pages_form.tpl'          => '~base/templates/pages_form.tpl',
                        'templates/toolbar.tpl'             => '~base/templates/toolbar.tpl',
                        // Locales
                        'templates/lang/_client.lng'             => '~base/templates/lang/_client.lng',
                        'templates/lang/_form.lng'               => '~base/templates/lang/_form.lng',
                        'templates/lang/_menu_all.lng'           => '~base/templates/lang/_menu_all.lng',
                        'templates/lang/_menu_owners.lng'        => '~base/templates/lang/_menu_owners.lng',
                        'templates/lang/eshop_cat_form.lng'      => '~base/templates/lang/eshop_cat_form.lng',
                        'templates/lang/eshop_item_form.lng'     => '~base/templates/lang/eshop_item_form.lng',
                        'templates/lang/eshop_order_form.lng'    => '~base/templates/lang/eshop_order_form.lng',
                        'templates/lang/ext_image.lng'           => '~base/templates/lang/ext_image.lng',
                        'templates/lang/mod.lng'                 => '~base/templates/lang/mod.lng',
                        'templates/lang/toolbar.lng'             => '~base/templates/lang/toolbar.lng',
                        'templates/lang/pages_form.lng'          => '~base/templates/lang/pages_form.lng',
                    )
                );
        }
        $basePath = '_shared/code/skins/' . $name;
        $localPath = '_local/skins/' . $name;
        if(!isset($aSkin['path'])){
            $aSkin['path'] = array();
        }
        $aSkin['path'] += array(
        ...
        $aSkin = self::updatePaths($aSkin);
        self::$aSkins[$name] = $aSkin;
    }

    /**
     * Checks if skin was added.
     *
     * @param  string  $name       Skin name
     * @param  bool   $throwError  Trigger error if skin was not added
     * @return bool
     * @amidev Temporary
     */
    public static function exists($name, $throwError = FALSE){
        $res = isset(self::$aSkins[$name]);
        if($throwError && !$res){
            trigger_error('Skin "' . $name . '" is not defined.', E_USER_WARNING);
        }
        return $res;
    }
     ...
     * @amidev Temporary
     */
    public static function update($name, $aSkin){
        if(AMI_Skin::exists($name, TRUE)){
            $aSkinOrigin = self::$aSkins[$name];
            self::$aSkins[$name] = self::updatePaths(array_merge_recursive($aSkinOrigin, $aSkin));
        }
    }

    /**
     * Returns true if path to skin exists.
     *
     * @param string $filePath  File resource path
     * @return boolean
     */
    public static function issetPath($filePath){
        return isset(self::$aSkins[self::$current]['path'][$filePath]);
    }

    /**
     * Returns real path to skin file resource.
     *
     * @param string $filePath  File resource path
     * @return boolean
     */
    public static function getPath($filePath){
        return self::issetPath($filePath) ? self::$aSkins[self::$current]['path'][$filePath] : FALSE;
    }

    /**
     * Returns active skin base path.
     *
     * @return string
     * @amidev Temporary
     */
    public static function getBasePath($name = FALSE){
        if($name === FALSE){
            $name = self::$current;
        }
        return self::$aSkins[$name]['path']['base'];
    }

    /**
     * Returns active skin local path.
     *
     * @return string
     */
    public static function getLocalPath($name = FALSE){
        if($name === FALSE){
            $name = self::$current;
        }
        return self::$aSkins[$name]['path']['local'];
    }
    ...
    public static function get($name = FALSE){
        ...
        return AMI_Skin::exists($name, TRUE) ? self::$aSkins[$name] : array();
    }
    ...
    /**
     * Updates ~base and ~local in pathnames.
     *
     * @param array $aSkin  Original skin array
     * @return array
     * @amidev
     */
    protected static function updatePaths(array $aSkin){
        if(isset($aSkin['path']) && is_array($aSkin['path'])){
            $basePath = $aSkin['path']['base'];
            $localPath = $aSkin['path']['local'];
            foreach($aSkin['path'] as $resource => $path){
                if(strpos($path, '~base/') !== FALSE){
                    $aSkin['path'][$resource] = str_replace('~base/', $basePath . '/', $path);
                }elseif(strpos($path, '~local/') !== FALSE){
                    $aSkin['path'][$resource] = str_replace('~local/', $localPath . '/', $path);
                }
            }
        }
        return $aSkin;
