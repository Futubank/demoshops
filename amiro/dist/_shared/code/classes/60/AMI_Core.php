 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       37955 xkqwmpksznnkmriyptxuppqqkkxtqglgwwwiyirsitgmmgmxymzmsyuguxtyxylipxzmpnir
 */
?><?php
...
interface AMI_iCore{
    ...
    /**
     * Returns core module lazy option value.
     *
     * @param  mixed $name  Module option name
     * @return mixed
     */
    public function getOptionLazy($name);
    ...
    /**
     * Sets core module lazy option value.
     *
     * @param  string $name  Module option name
     * @param  mixed $value  Module option value, null to unset
     * @return void
     */
    public function setOptionLazy($name, $value);
    ...
    /**
     * Returns true if core module lazy option is set.
     *
     * @param  string $name  Module options name
     * @return bool
     */
    public function issetOptionLazy($name);
...
final class AMI_Core implements AMI_iCore{
     ...
     * @since  7.0.0
     */
    public function getModOptionLazy($modId, $name){
        return $this->oOptions->getModParamLazy($modId, $name);
    }
     ...
     * @since  7.0.0
     */
    public function setModOptionLazy($modId, $name, $value){
        $this->oOptions->setModParamLazy($modId, $name, $value);
    }
    ...
    /**
     * Returns true if module lazy option is set.
     *
     * @param  string $modId  Module Id
     * @param  string $name   Module options name
     * @return bool
     * @since  7.0.0
     */
    public function issetModOptionLazy($modId, $name){
        // if(!is_object($this->oOptions))die(d::getTraceAsString());
        return $this->oOptions->issetModParamLzy($modId, $name);
    }
     ...
     */
    public function getOptionLazy($name){
        return $this->oOptions->getModParamLazy('core', $name);
    }
     ...
     */
    public function setOptionLazy($name, $value){
        $this->oOptions->setModParamLazy('core', $name, $value);
    }
     ...
     */
    public function issetOptionLazy($name){
        return $this->oOptions->issetModParamLazy('core', $name);
    }
     ...
     * @amidev
     */
    public function expireUserSession($id){
        $oSess = admSession();
        if($oSess){
