 ...
 * @version    $Id$ 
 * @since      5.10.0 
 * @size       21395 xkqwipprygqmgqkmkyzwgimwqqkypzwptmtnxuxkmqwmuppiytqukwnuruqwnumpxrslpnir
 */
?><?php
...
class AMI_Debug{
    ...
    public static function getIncludedFilesHTML(){
        ...
        $html =
            "<a class=\"deb\" href=\"javascript:\" onclick=\"var _tabIncFiles = document.getElementById('tableIncludedPHPFiles{$count}');_tabIncFiles.style.display = _tabIncFiles.style.display == 'none' ? 'block' : 'none'; return false;\">{$count}. included files  (" . sizeof($files) . ")</a> " .
            "<table id=\"tableIncludedPHPFiles{$count}\" cellpadding=\"2\" cellspacing=\"2\" border=\"1\" class=\"deb\" style=\"display: none;\">\n";
        $sum = 0;
...
class d extends AMI_Debug{
     ...
     * @since  7.0.0
     */
    public static function vel($entity, $title = ''){
        trigger_error(
            "var_export" . ('' === $title ? '' : '[' . $title . ']') . ":\n" .
