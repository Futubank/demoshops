 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       20954 xkqwkynuttswkwwgtrgzmpqrisxrqturkgmstnglnzknrukuyxzksuswwwsulywuruxypnir
 */
?><?php
...
final class AMI_ErrorHandler{
    ...
    */
    ...
    public function handleError($code, $message, $file, $line, array $aTrace = null){
        ...
        if(($code & E_NOTICE) && AMI_PROC_API_E_NOTICES){
            // Process norices from API code only

            static $aPathRegExps = array(
                '~^_shared/code/classes/60$~',
                '~^_shared/code/hyper_modules/~',
                '~^_local~'
            );
            ...
            $nonAPINotice = TRUE;
            foreach($aPathRegExps as $pathRegExp){
