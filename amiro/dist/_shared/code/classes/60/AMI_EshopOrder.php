 ...
 * @version    $Id$ 
 * @since      5.14.8 
 * @size       17965 xkqwizzlgqxzpzmrssryysitutzrsrlpryrylullsqsksknilziiyttuisywipumipzypnir
 */
?><?php
...
class AMI_EshopOrder{
    ...
    public function getOrderData(Ami_ModTableItem $oOrder){
        $aResult = array(
            ...
            'payment_method_fee'       => 0, // Payment tax
        ...
        if(is_array($aSysInfo)){
            if(isset($aSysInfo['fee_percent'])){
                $paymentMethodFee = $amount * $aSysInfo['fee_percent'] / 100;
                $feeConst = $oEshop->convertCurrency($aSysInfo['fee_const'], $aSysInfo['fee_curr'], $this->defaultCurrency);
                $paymentMethodFee += $feeConst;
                $aResult['payment_method_fee'] = $oEshop->formatNumber($paymentMethodFee, TRUE, TRUE);
            }
        }
