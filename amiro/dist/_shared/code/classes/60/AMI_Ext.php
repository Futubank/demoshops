 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       17798 xkqwywwiipwpwigtupzxpmxqzxmknkixxznqnynmitgpxusknykylyrqmywsuwuzgggwpnir
 */
?><?php
    ...
    */
    ...
    public static function getAviableModExtensions($modId){
        ...
        foreach($aModNames as $name){
            $isExt = TRUE;
            if($oDeclarator->isRegistered($name)){
                list($extHyper, ) = $oDeclarator->getHyperData($name);
                $isExt = 'ami_ext' === $extHyper;
            }
            
            // dirty hardcode for 'ce_page_break' extension, because it ain't start from 'ext_'
            if($isExt && (preg_match('/^ext_/', $name) || $name == 'ce_page_break')){
                $aAllExt[] = $name;
