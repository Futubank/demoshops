 ...
 * @version    $Id$ 
 * @since      5.14.4 
 * @size       7186 xkqwwwsyilxntpziiiqluuqxsqkntpgiuxltrzuzgzzipttqmkkygntgllmmrssrmqrqpnir
 */
?><?php
...
class AMI_Lib_Array{
     ...
     * @return void
     */
    public static function sortMultiArray(array &$aArray, $sortCol, $sort = SORT_STRING, $direction = SORT_ASC){
        if(sizeof($aArray) > 0){
            $aIndex = array();
            $aRes = array();
            $i = 0;
     ...
     *
     * @param  array $aFirst   First array
     * @param  array $aSecond  Second array
     * @return array
     * @see    http://php.net/manual/en/function.array-diff-assoc.php
     * @since  7.0.0
     */
    public static function diffAssoc(array $aFirst, array $aSecond){
        $aFirstCopy = $aFirst;
        $aSecondCopy = $aSecond;
        $aReplaced = array();
        foreach($aFirst as $key => $value){
            if(is_array($value)){
                $aFirstCopy[$key] = serialize($value);
                $aReplaced[] = $key;
            }
        }
        foreach($aSecond as $key => $value){
            if(is_array($value)){
                $aSecondCopy[$key] = serialize($value);
            }
        }
        $aDiff = array_diff_assoc($aFirstCopy, $aSecondCopy);
        foreach($aReplaced as $key){
            if(isset($aDiff[$key])){
                $aDiff[$key] = unserialize($aDiff[$key]);
            }
        }
