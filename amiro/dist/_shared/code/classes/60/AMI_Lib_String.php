 ...
 * @version    $Id$ 
 * @since      5.12.0 
 * @size       10481 xkqwrnixxswtkkuimnmqxrqtskziwrkiswrilniqxgkxlrntkmrlnpyulgruuiilrrtipnir
 */
?><?php
...
*/
     ...
     * @param  bool   $isRev   Reverse output
     * @return string
     */
    public static function hex2Bin($hex, $isRev = true){
        $out = "";
        if(preg_match('/^(0x)?([0-9a-fA-F]+)$/i', $hex, $matches)){
            for($i = 0; $i < mb_strlen($matches[2]); $i++){
                $out .= str_pad(decbin(hexdec($matches[2][$i])), 4, "0", STR_PAD_LEFT);
            }
        }
        if($isRev)
            $out = strrev($out);
     ...
     *
     * @param  string $string      String
     * @param  bool   $withSpaces  Convert "&nbsp;" character
     * @param  bool   $withQuotes  Convert "#039;", "&#037;", "&#035;" characters (since 7.0.0)
     * @return string
     * @todo   Remember to change ami.common.js decodeHTMLSpecialChars when expanding replacements
     * @amidev
     */
    public static function unhtmlEntities($string, $withSpaces = FALSE, $withQuotes = TRUE){
        static $aTable, $aTableSpaces, $aTableQuotes, $aTableSpacesQuotes;

        if(empty($aTable)){
            $aTrans = get_html_translation_table(HTML_SPECIALCHARS);
            $aTable = array_flip($aTrans);
            $aTableSpaces = array_flip($aTrans + array(' ' => '&nbsp;'));
            $aTableQuotes = array_flip($aTrans + array("'" => '&#039;', '%' => '&#037;', '#' => '&#035;'));
            $aTableSpacesQuotes = array_flip($aTrans + array(' ' => '&nbsp;', "'" => '&#039;', '%' => '&#037;', '#' => '&#035;'));
        }
        if($withSpaces && $withQuotes){
            $aTrans = $aTableSpacesQuotes;
        }elseif($withQuotes){
            $aTrans = $aTableQuotes;
        }elseif($withSpaces){
            $aTrans = $aTableSpaces;
