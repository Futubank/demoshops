 ...
 * @version    $Id$ 
 * @since      5.12.0 
 * @size       37214 xkqwzigwkmwnnuqtugxtiyiuislngumnwsiyszpwpmylmkwyqnltyrzwgkysmwmsgnwkpnir
 */
?><?php
...
abstract class AMI_Mod{
     ...
     * @since 7.0.0
     */
    const SIGN_EXPIRATION = 7; // in days
    ...
    public function __construct(AMI_Request $oRequest, AMI_Response $oResponse){
        ...
        if($this->statusMessagePath === ''){
            ...
            if(strpos($filename, 'plugins') === false){
                // Modules
                $this->statusMessagePath =
                    (AMI_Registry::get('side') != 'adm' ? AMI_iTemplate::LNG_MOD_PATH : AMI_iTemplate::LOCAL_LNG_MOD_PATH) .
                    '/' . $modId . '_messages.lng';
            }else{
                // Plugins
    ...
    public function dispatchRequest($name, array $aEvent, $handlerModId, $srcModId){
         ...
         */
        $oRequest = AMI::getSingleton('env/request');
        $action = $oRequest->get('mod_action', 'none');

        // #CMS-11715 {
        $oSession = NULL;
        if(function_exists('admSession')){
            // admin session
            $oSession = admSession();
        }elseif(isset($GLOBALS['oSession'])){
            // front session
            $oSession = $GLOBALS['oSession'];
        }
        $dispatch = TRUE;
        if(
            (AMI_Registry::get('side') != 'adm') && /* TEMPORARY */
            is_object($oSession) && $oSession->IssetVar('hash') &&
            !in_array($action, array('none', 'form_edit')) &&
            FALSE === mb_strpos($action, 'view')
        ){
            $ts = (int)$oRequest->get('_amits', 0);
            $hash = $oRequest->get('_amitsh', FALSE);
            /**
             * AMI_Response
             */
            $oResponse = AMI::getSingleton('response');
            if(!$ts || FALSE === $hash){
                trigger_error('Missing action sign', E_USER_WARNING);
                $oResponse->setMessage('status_action_sign_missing', AMI_ModForm::SAVE_FAILED);
                $oResponse->addStatusMessage('status_action_sign_missing', array(), AMI_Response::STATUS_MESSAGE_ERROR);
                $dispatch = FALSE;
            }elseif((time() - $ts) > (self::SIGN_EXPIRATION * 60 * 60 * 24)){
                trigger_error('Action sign expired', E_USER_NOTICE);
                $oResponse->setMessage('status_action_sign_expired', AMI_ModForm::SAVE_FAILED);
                $oResponse->addStatusMessage('status_action_sign_expired', array(), AMI_Response::STATUS_MESSAGE_ERROR);
                $dispatch = FALSE;
            }elseif(md5($ts . $oSession->GetVar('hash')) !== $hash){
                trigger_error('Invalid action sign', E_USER_WARNING);
                $oResponse->setMessage('status_action_sign_invalid', AMI_ModForm::SAVE_FAILED);
                $oResponse->addStatusMessage('status_action_sign_invalid', array(), AMI_Response::STATUS_MESSAGE_ERROR);
                $dispatch = FALSE;
            }
        }
        if(!$dispatch){
            if('form_save' !== $action){
                $aAction = explode('_', $action);
                if(sizeof($aAction) > 1){
                    $action = $aAction[0] . '_view';
                }
            }else{
                $action = 'form_edit';
            }
            $oRequest->set('mod_action', $action);
            $oRequest->setScope(array(), 'p');
        }
