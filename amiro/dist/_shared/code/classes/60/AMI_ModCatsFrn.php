 ...
 * @version    $Id$ 
 * @since      5.14.8 
 * @size       27889 xkqwrxrgnmwxtrpnykpzmysttuiskrszmguzkmriqigwyppmlriutguyxlqsnitwqgkkpnir
 */
?><?php
...
abstract class AMI_ModCatsFrn extends AMI_ModList{
    ...
    public function init(){
        ...
        if(!AMI::editModeEnabled() && AMI::issetAndTrueProperty($modId . '_cat', 'use_special_list_view')){
    ...
    public function handleSubitems($name, array $aEvent, $handlerModId, $srcModId){
        ...
        if(sizeof($this->aCatIds)){
            ...
            // #CMS-11745: empty($_REQUEST['ami_efe'])
            if(!AMI::editModeEnabled() && AMI::issetAndTrueProperty($modId, 'use_special_list_view')){
    ...
    protected function displayAllSubitems(array &$aEvent){
        ...
        if(empty($this->oSubitems)){
            ...
            // #CMS-11745: empty($_REQUEST['ami_efe'])
            $useSpecListView =
                AMI::issetAndTrueProperty($modId, 'use_special_list_view') &&
                AMI::doFilterFrn($modId);
            if($useSpecListView){
                $this->oSubitems->addWhereDef(
                    'AND (i.`' . $oTable->getFieldName('hide_in_list') . '` = 0 ' .
                    'OR i.`' . $oTable->getFieldName('sticky') . '` = 1' . AMI_GlobalFilters::addRightsFilterAddon($modId, 'i') . ')'
                );
            }
    ...
    protected function displayLimitedSubitems(array &$aEvent, $subitems){
        ...
        // Filter by hidden/sticky items
        // #CMS-11745: empty($_REQUEST['ami_efe'])
        $useSpecListView = /* !AMI::editModeEnabled() && */AMI::issetAndTrueProperty($modId, 'use_special_list_view');
        if($useSpecListView){
            $oList->addWhereDef(
                'AND (i.`' . $oTable->getFieldName('hide_in_list') . '` = 0 ' .
                'OR i.`' . $oTable->getFieldName('sticky') . '` = 1' . AMI_GlobalFilters::addRightsFilterAddon($modId, 'i') . ')'
...
abstract class AMI_ModCatsView extends AMI_ModListView{
    ...
    public function get(){
        ...
        */
        ...
        if(
            !AMI::getOption($modId . '_cat', 'show_empty_cats') &&
            (!AMI::editModeEnabled() || AMI_Registry::get('ami_specblock_mode', FALSE))
        ){
        ...
        if($this->oList->count() > 0){
            ...
            foreach($this->oList as $oItem){
                ...
                $rowTpl = $this->oCommonView->parseTpl('row', $aItem);
                if(AMI::editModeEnabled() && !AMI_Registry::get('ami_specblock_mode', false)){
                    if(AMI::checkAccessRights($this->getModId(), $oItem->id_owner, array('edit'))){
                        $aIconsData = array(
                            'id' => $aItem['id'],
                            'isCat' => 'true',
                            'modId' => $modId,
                            'header' => AMI_Lib_String::htmlChars($oItem->header)
                        );
                        $this->getTemplate()->addBlock('list_icons', AMI_Skin::getPath('templates/list_icons.tpl'));
                        if((mb_strpos($rowTpl, '<!--AMI_ROW_ICONS-->') === FALSE) && strpos($rowTpl, '<td') === FALSE){
                            $icons = $this->getTemplate()->parse('list_icons:icons', $aIconsData);
                            $rowTpl = $icons . $rowTpl;
                        }else{
                            $aIconsData['isCat'] = true;
                            $jsonData = json_encode($aIconsData);
                            $marker = '<!--AMI_ROW_EDITABLE:' . $jsonData . '-->';
                            if(mb_strpos($rowTpl, '<!--AMI_ROW_ICONS-->') !== false){
                                $rowTpl = str_replace('<!--AMI_ROW_ICONS-->', $marker, $rowTpl);
                            }else{
                                $rowTpl = $marker . $rowTpl;
                            }
                        }
                    }
                }
                $aData['browser_row'] = $rowTpl;
    ...
    protected function setFilter(){
        ...
        if($aBrowser['useSpecView']){
            $this->oList->addWhereDef(
                DB_Query::getSnippet(
                    "AND (`i`.`" . $this->oModel->getFieldName('hide_in_list') . "` = 0"  . AMI_GlobalFilters::addRightsFilterAddon($this->getModId(), 'i') . ')'
                )
            );
        }
