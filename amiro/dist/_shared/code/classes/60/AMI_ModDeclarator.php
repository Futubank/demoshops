 ...
 * @version    $Id$ 
 * @since      5.14.4 
 * @size       46229 xkqwyzzgtlupwrnnnlwlnwyizzlipssmxwwkrxrrysmuggnnxzxsyqppxzwguqymsruypnir
 */
?><?php
...
final class AMI_ModDeclarator{
    ...
    public function register($hypermod, $config, $modId, $parentModId = '', $flags = self::INTERFACE_ADMIN){
        ...
        $hasCore = is_object($this->oCore);
        if($hasCore){
            $oMod = $this->oCore->declareModule(
                $this->lastSection,
                $hypermod,
                $config,
                $modId,
                $parentModId,
                (bool)($flags & self::INTERFACE_FRONT)
            );
        }
...
*/
    ...
    private function __construct(){
        ...
        $this->useSharedConfigsOnly =
            !empty($sys['disable_user_scripts']) &&
            // #CMS-11790
            !defined('AMI_ALLOW_LOCAL_MODULES');
...
final class AMI_Module{
     ...
     * @param  int    $flags  Option flags (since 7.0.0)
     *                        I. e. self::OPT_LAZY
     * @return AMI_Module
     */
    public function setOption($name, $value, $flags = 0){
        if($this->allowToSetOptions){
            if($flags & self::OPT_LAZY){
                $this->oMod->setOptionLazy($name, $value);
            }else{
    ...
    public function addRule($name, $type, $aOptions = AMI_Module::RLC_NONE, $defaultValue = NULL, $aGroups = array()){
        if(in_array($type, $this->aRuleTypes)){
            ...
            if(is_object($this->oRuleMod)){
                $this->oRuleMod->addRule(
                    CMS_CoreRules::RLR_ANY,
