 ...
 * @version    $Id$ 
 * @since      5.14.8 
 * @size       10026 xkqwgkmntnurmwgzkzppnqrmnqzkkswiktwkxiwwmklwwiiituxylliyzsxxnyixlltqpnir
 */
?><?php
    ...
    public function get(){
        ...
        if(isset($aPage['itemId']) && $aPage['itemId'] > 0){
            ...
            if(AMI::editModeEnabled()){
                ...
                $aRights = AMI::getAccessRights($this->getModId(), $oUser->id);
                if(
                    !in_array('system_user', $aRights['roles']) &&
                    !in_array('moderator', $aRights['roles']) &&
                    $oUser->id != $oItem->id
                ){
                    AMI_Registry::set('ami/editMode/discardEditingDetails', TRUE);
                }
            }
        ...
        if(isset($aScope['header'])){
            ...
            if(AMI::editModeEnabled()){
                if(AMI::checkAccessRights($this->getModId(), $oItem->id_owner, array('edit'))){
                    AMI_Registry::get('oGUI')->addHtmlScript('var active_module_item_header = "' . AMI_Lib_String::jParse($aScope['header']) .'";');
                }
            }
        }
