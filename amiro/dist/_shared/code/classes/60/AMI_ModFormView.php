 ...
 * @version    $Id$ 
 * @since      5.12.0 
 * @size       48827 xkqwytmlxrkmpgimknssutpmsrnrqgkznlwitmpwwpgpnsykwpwxqnsryntyktkuummspnir
 */
?><?php
...
abstract class AMI_ModFormView extends AMI_ModPlaceholderView implements AMI_iModFormView{
    ...
    public function get(){
        ...
        if(AMI_Registry::get('ami/module/form/forceJSONResponse', FALSE) === TRUE){
    ...
    protected function getSectionHTML($sectionName, array $aSectionContent){
        ...
        if(isset($this->aTabs[$sectionName])){
            $prefix = 'tab';
            foreach($this->aTabContainers as $container => $aContainerTabs){
                foreach($aContainerTabs['tabs'] as $tabName => $aTab){
                    if($sectionName == $tabName){
                        $aScope['state'] = $aTab['state'];
                        break;
                    }
                }
            }
    ...
    protected function getFieldHTML($fieldName){
        ...
        $formField = '';
        if(isset($aField['html'])){
            // Don't generate html if 'html' already set
            $formField = $aField['html'];
        }elseif(is_array($aFieldScope)){
            $formField = $this->oTpl->parse($this->tplBlockName . ':' . $aField['type'] . '_field', $aFieldScope);
        }
    ...
    /**
     * Returns field scope for render.
     *
     * @param  &string $fieldName  Field name
     * @return array or NULL
     * @amidev
     */
    protected function getFieldScope(&$aField){
        ...
        }elseif(!isset($aFieldScope['element_caption'])){
            if(in_array($aFieldScope['type'], array('select', 'checkbox', 'radio', 'input'))){
                $aFieldScope['element_caption'] = '--- ' . $caption . ' ---';
                if('' != $caption){
                    trigger_error('Language variable `' . $viewType . '_field_' . $caption . '` not found', E_USER_NOTICE);
                }
            }
        }
    ...
    protected function getFormButtons(){
        ...
        if($this->viewType == 'form'){
            $type =
                $this->oItem && $this->oItem->getId()
                ? 'edit'
                : 'new';
            $this->aFormButtons = array_diff($this->aFormButtons, $this->aExcludedButtons[$type]);
            $aButtonsData = array();
            foreach($this->aFormButtons as $btnName){
                $aButtonsData[$btnName . '_btn'] = $this->oTpl->parse(
                    $this->tplBlockName . ':' . $btnName . '_btn',
                    $this->aScope
                );
            }
            $formButtons = $this->oTpl->parse(
                $this->tplBlockName . ':' . 'form_buttons',
                $aButtonsData
            );
        }
...
/*
            ...
            if(
                ...
                !in_array($placeholder, $this->getTabNames($container))
            ){
                // trigger_error('placeholder ' . $placeholder);trigger_error('container ' . $container);trigger_error(var_export($this->getTabNames($container), TRUE));
...
*/
     ...
     */
     ...
     /**
      * Returns added tab names.
      *
      * @return array
      * @amidev Temporary
      */
     protected function getTabNames(){
         return array_keys($this->aTabs);
     }
