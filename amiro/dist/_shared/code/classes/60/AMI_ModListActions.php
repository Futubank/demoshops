 ...
 * @version    $Id$ 
 * @since      5.12.0 
 * @size       40161 xkqwxyyrmqllssqnrpgiwggkqwugunuuykzpisrtskrwyikxrzxligzyilgniplqrikgpnir
 */
?><?php
    ...
    public function dispatchGrpMovePosition($name, array $aEvent, $handlerModId, $srcModId){
        ...
        if($nMoved > 0){
            ...
            if($moveType == 'move_top'){
                ...
                if($oTable->hasField('id_cat')){
                    $oQuery->addWhereDef(
                        " AND `"  . $oTable->getFieldName('id_cat') . "` = " . $catId
                    );
                }
                $this->localeCondition($oTable, $oQuery);
                ...
                if($oTable->hasField('id_cat')){
                    $oQuery
                        ->addWhereDef("AND `" . $oTable->getFieldName('id_cat') . "` = " . $catId);
                }
                $pos = $oDB->fetchValue($oQuery);
                if($pos){
                    $posAfter = $pos;
