 ...
 * @version    $Id$ 
 * @since      5.10.0 
 * @size       55432 xkqwullzxukxuxpxiugszyuzmzuyqyiusuxtswmwslrkitkxtxxpynrzplwsztysuxmppnir
 */
?><?php
...
abstract class AMI_ModTableItem implements AMI_iModTableItem, Iterator, AMI_iFormModTableItem{
    ...
    public function load(){
        ...
        if($hasCondition){
            ...
            $this->oQuery->setLimitParameters(0, 1);
            $aRecord = $oDB->fetchRow($this->oQuery);
        }
        if($aRecord){
            $this->setData($aRecord);
            $this->id = isset($aRecord[$this->primaryKeyField]) ? $aRecord[$this->primaryKeyField] : NULL;
            if($this->oTable->hasField('sm_data') && isset($aRecord['sm_data'])){
                $aMeta = @unserialize($aRecord['sm_data']);
                if(is_array($aMeta)){
                    if(!AMI_Registry::get('AMI/Module/Model/skipHTMLDecoding', FALSE)){
                        $aMeta['title'] = AMI_Lib_String::unhtmlEntities($aMeta['title'], FALSE, FALSE);
                    }
...
*/
    ...
    public function setData(array $aData, $bAppend = false, $bSetPK = false){
        if(isset($aData['sm_data'])){
            ...
            if(!AMI_Registry::get('AMI/Module/Model/skipHTMLDecoding', FALSE)){
                $aHTMLFields['title'] = AMI_Lib_String::unhtmlEntities($aHTMLFields['title']);
            }
            unset($aData['sm_data']);
        ...
        if($this->oTable->getAttr('doRemapListItems', FALSE)){
            ...
            if(!$this->oTable->getAttr('keepOldDataOnRemap', FALSE)){
                $this->aData = array();
            }
            foreach($aData as $name => $value){
                if(!is_numeric($name)){
    ...
    /**
     * Encoding/decoding '%' symbol field callback.
     *
     * @param  array $aData  Formatter data
     * @return array
     */
    private function fcbEncodedPercent(array $aData){
        $action = $aData['action'];
        switch($action){
            case 'get':
                if(!AMI_Registry::get('AMI/Module/Model/skipHTMLDecoding', FALSE)){
                    $aData['value'] = str_replace('&#037;&#037;', '%%', $aData['value']);
                }
                // $e = new Exception;###
                // trigger_error('get ' . $aData['name'] . ': ' . $aData['value']/* . "\n" . $e->getTraceAsString() . "\n\n"*/);###
                break;
            case 'set':
                $aData['value'] = str_replace('%%', '&#037;&#037;', $aData['value']);
                // $e = new Exception;###
                // trigger_error('set ' . $aData['name'] . ' to ' . $aData['value']/* . "\n" . $e->getTraceAsString() . "\n\n" */);###
                break;
        }
    ...
    /**
     * Auto HTML-special chars field callback.
     *
     * @param  array $aData  Formatter data
     * @return array
     */
    private function fcbHTMLSpecialChars(array $aData){
        $action = $aData['action'];
        switch($action){
            case 'get':
                $aData['value'] = AMI_Lib_String::unhtmlEntities($aData['value'], FALSE, FALSE);
                break;
            case 'set':
                $aData['value'] = htmlspecialchars($aData['value']);
                break;
        }
