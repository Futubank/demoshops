 ...
 * @version    $Id$ 
 * @since      5.14.4 
 * @size       26551 xkqwznqytztumruzpuynulgzznwmugrgpusrwgrttwzziqznmnyxslqluxpqrqpntygtpnir
 */
?><?php
...
class AMI_ModTableItemMeta implements AMI_iModTableItemMeta{
    ...
    public function handleSaveModelItem($name, array $aEvent, $handlerModId, $srcModId){
        ...
        if($aEvent['oTable']->hasField('sm_data')){
            /*
            $aEvent['aData']['html_title'] = str_replace(
                array('&#039;', '&quot;'),
                array("'", '"'),
                $aEvent['aData']['html_title']
            );
            */
        ...
        if(empty($sublink)){
            ...
            if(!$this->checkURLSymbols($sublink)){
                ...
                if(
                    $aEvent['aData']['id'] &&
                    !empty($origSublink) &&
                    $this->checkURLSymbols($origSublink)
                ){
                    // Restore original link
                    $sublink = $origSublink;
                }else{
                    // Generate link
                    $aEvent['aData']['sublink'] = '';
                    $this->processSublink($aEvent);
                }
            }else{
    ...
    public function addItemIdToSublink($name, array $aEvent, $handlerModId, $srcModId){
        // $aEvent['aData'] contains mapped 'sublink' field name, doesn't work for 'pages/table/item' model
    	// $aEvent['oItem']->sublink = $aEvent['aData']['sublink'] . '-' . $aEvent['oItem']->id;
    	$aEvent['oItem']->sublink = $aEvent['oItem']->sublink . '-' . $aEvent['oItem']->id;
        if($this->oConflictItem){
            $oItem = $aEvent['oTable']->find($aEvent['oItem']->id);
            $oItem->sublink = $aEvent['oItem']->sublink;
            ...
            AMI::getSingleton('response')
                ->addStatusMessage(
                    ...
                    array(
                        'front_link' => AMI_Registry::get('path/www_root') . $this->oConflictItem->getFullURL(),
                        'new_link'   => AMI_Registry::get('path/www_root') . $oItem->getFullURL(),
    ...
    protected function processHTMLMeta(array &$aEvent){
        ...
        if(
            ...
            if(
                ...
                $aData = array();
                foreach($aEvent['oTable']->getHTMLFields() as $key => $field){
                    str_replace('"', '', $aHTMLMeta['keywords']);
            ...
            $aHTMLMeta = array(
                ...
                'keywords'     => str_replace('"', '', $aEvent['aData']['html_keywords']),
                'description'  => str_replace('"', '', $aEvent['aData']['html_description']),
                'is_kw_manual' => '1',
...
//            trigger_error($this->storeAsSnippet . ' ' . var_export($aHTMLMeta, TRUE));###
            if($this->storeAsSnippet){
                ...
                foreach(
                    ...
                    if(in_array($key, array('keywords', 'description'))){
                        $aHTMLMeta[$key] = str_replace('"', '', $aHTMLMeta[$key]);
                    }
                    $aEvent['aData'][$field] = $aHTMLMeta[$key];
                }
            }
        }
    }
