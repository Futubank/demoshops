 ...
 * @version    $Id$ 
 * @since      5.12.4 
 * @size       28475 xkqwuwrxxnuzxznplpxygkxyntrnklpskxyrzyqrxssgxlgrlpqiwymuimgwlkwpgwrmpnir
 */
?><?php
...
abstract class AMI_ModTableItemModifier{
    ...
    public function save(){
         ...
         * @eventparam bool              _discard   Allows to discard saving using AMI_ModTableItemModifier::rollback() (since 7.0.0)
         */
        AMI_Event::fire('on_before_save_model_item', $aEvent, $this->oTableItem->getModId());
        if(!empty($aEvent['_discard'])){
            trigger_error("Saving discarded during event 'on_before_save_model_item' for module Id '" . $this->oTableItem->getModId() . "'");
            $this->rollback($aData, $onCreate);
...
*/
    ...
    public function delete($id = null){
        ...
        if($result && isset($oItem)){
            foreach($aFileFields as $field => $params) {
                $aSearchCondition = array();
                foreach(array_keys($params['aParams']['aMapping']) as $mapped) {
                    $aSearchCondition[$mapped] = $oItem->getValue($mapped);
                }
                $oSearchItem = $this->oTable->findByFields($aSearchCondition);
                if(!$oSearchItem->id) {
