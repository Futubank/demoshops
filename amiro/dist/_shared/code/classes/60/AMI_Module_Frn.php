 ...
 * @version    $Id$ 
 * @since      5.14.8 
 * @size       23429 xkqwupwpqrtuwnglkqrqirxpysrsqwrqgxkrptugylpummnixmgirmwqwzykwyxwwrqypnir
 */
?><?php
...
abstract class AMI_Module_Frn extends AMI_Mod{
     ...
     * @var bool
     */
    protected $discardChildInit = FALSE;
    ...
    public function __construct(AMI_Request $oRequest, AMI_Response $oResponse){
        ...
        if(FALSE !== $oRequest->get('ami_asnc', FALSE)){
            AMI_Skin::setupModuleSpecialMode($modId);
        }else{
    ...
    public function processSEO(){
        ...
        if(!$this->isPageForceIndexing && $this->isPageIndexing){
            ...
            if (AMI::getSingleton('env/request')->getCookie('forceMobile', false)) {
                unset($aRequest['lay_id']);
                unset($aGETVars['lay_id']);
            }
    ...
    public function processCanonical() {
        $sBaseHref = AMI_Registry::get('path/www_root');
        $aLinks = AMI_Registry::get('page/links', array());
        $aGETVars = array();
        $aURI = parse_url(AMI::getSingleton('env/request')->getURL('uri'));
        if(isset($aURI['query'])){
            parse_str($aURI['query'], $aGETVars);
        ...
        $pKeyName = AMI::getOption('core', 'print_flag');
        if(
            empty($aLinks['canonical']) &&
            isset($aGETVars[$pKeyName]) && (bool)$aGETVars[$pKeyName] == true
        ) {
            $query = '';
            foreach($aGETVars as $key => $value) {
                if(mb_strlen($key) > 0 && !in_array($key, array($pKeyName, 'submit_url'))) {
                    $query .= (mb_strlen($query) > 0 ? '&' : '?') . $key . '=' . $value;
                }
            }
            $aLinks['canonical'] = $aURI['path'] . $query;
        }
        if(isset($aLinks['canonical'])) {
            unset($aLinks['prev']);
            unset($aLinks['next']);
        }

        $oGUI = AMI_Registry::get('oGUI');
        foreach($aLinks as $type => $value) {
            $oGUI->setLink(_fmtHrefLink((mb_strpos($value, $sBaseHref) === false ? $sBaseHref : '') . $value, ''), $type);
        }
        return $this;
    }
