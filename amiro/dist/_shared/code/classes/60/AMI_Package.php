...
class AMI_Tx_Cmd_Package_OnPostInstall extends AMI_Tx_Cmd{
    ...
    public function commit(){
        ...
        AMI_Package_InstanceManipulator::$aProcessedEntities[] = $this->oArgs->processingEntity;
...
class AMI_Package{
    ...
    public static function getAvailableHyper($doLoadMeta = FALSE){
        ...
        if(empty($GLOBALS['sys']['disable_user_scripts']) || defined('AMI_ALLOW_LOCAL_MODULES')){
...
final class AMI_ModInstall extends AMI_Package_InstanceManipulator{
    ...
    protected function finish(){
        $hypermod = $this->aPkgInfo['install'][0]['hypermodule'];
        $config = $this->aPkgInfo['install'][0]['configuration'];
        $oRequest = AMI::getSingleton('env/request');
        if($GLOBALS['Core']->isInstalled('sys_groups')){
            $this->updateSysRights();
        }elseif(
            $oRequest->get('edit_at_front', FALSE) &&
            'ami_multifeeds' == $hypermod &&
            in_array($config, array('news', 'blog', 'articles', 'photoalbum'))
        ){

            // #CMS-11748 {

            $aModIds = AMI::getOption('members', 'edit_front_allowed_modules');
            $aModIds[] = $this->modId;
            AMI::setOption('members', 'edit_front_allowed_modules', $aModIds);
            AMI::saveOptions('members');
...
final class AMI_Package_Common{
    ...
    public function __construct($hypermod, $config, $pkgId, $installId = 0){
        ...
        if($this->hypermod === '' || $this->config === ''){
            if(
                is_array($this->aPkgInfo) &&
                isset($this->aPkgInfo['install']) &&
                isset($this->aPkgInfo['install'][0]['hypermodule']) &&
                isset($this->aPkgInfo['install'][0]['configuration'])
            ){
            ...
            if($this->hypermod === '' || $this->config === ''){
                $e = new Exception;
                trigger_error("Empty pakage info\n" . $e->getTraceAsString());
                return;
            }
        }
        ...
        if(AMI::validateModId($this->hypermod) && AMI::validateModId($this->config)){
            $aConfig =
                empty($GLOBALS['sys']['disable_user_scripts']) || defined('AMI_ALLOW_LOCAL_MODULES')
    ...
    public function loadStatusMessages($file){
        ...
        trigger_error(var_export($aLocale, TRUE));###
        */
        ...
        if($id){
            ...
            $sql =
                ...
                "SET " .
                    ...
                    "`content_type` = 0 " .
                "WHERE `id` = %s";
            $oQuery = DB_Query::getSnippet($sql)->q($content)->q('')->q($id);
            $oDB->query($oQuery, AMI_DB::QUERY_SYSTEM);
            AMI::getSingleton('response')
                ->addStatusMessage(
                    'import_locales_success',
                    array(
                        'src' => $this->sourceModId,
                        'dst' => $this->modId
                    )
                );
        }
...
final class AMI_PseudoPackage_Install extends AMI_PseudoPackage{
    ...
    protected function installAfter(){
        // Call install hook
        $path = $this->oPkgCommon->configPath . 'configs/' . $this->hypermod . '/' . $this->config . '/install_after.php';
        if(file_exists($path)){
            $this->oArgs = new AMI_Tx_Cmd_Args(get_object_vars($this));
            require $path;
        }
