 ...
 * @version    $Id$ 
 * @since      5.10.0 
 * @size       24163 xkqwstxrnqznwgwuxxmkkqwkmugngrrtsqrxisrrymmkwqyutykwytuzkinsskwptnywpnir
 */
?><?php
...
final class AMI_PageManager{
     ...
     * @amidev
     */
    private static $aDefaultPageIds;
...
*/
     ...
     * @amidev Temporary
     */
    public static function getPageMetaData(array $aMetaData){
        $res = array();
        ...
        if(is_array($GLOBALS['ModuleHtml']['headers'])){
    ...
    public static function initNavSettings($modId = ''){
        ...
        if(
            AMI::issetOption($modId, 'use_categories') && !AMI::getOption($modId, 'use_categories') &&
            ($idx = array_search('catid', self::$aNavModNames)) !== false
        ) {
            unset(self::$aNavModNames[$idx]);
        }
     ...
     *
     * @param  bool   $reload  Flag specifying to reload data from locales model
     * @return array  Array kontaining locales as keys and Ids as values
     */
    public static function getDefaultPageIds($reload = FALSE){
        if(is_null(self::$aDefaultPageIds) || $reload){
            self::$aDefaultPageIds = array();
            $oList =
                AMI::getResourceModel('locales/table')
                ->getList()
                ->addColumns(array('lang', 'default_id'))
                ->load();
            foreach($oList as $oItem){
                self::$aDefaultPageIds[$oItem->lang] = (int)$oItem->default_id;
            }
        }
        return self::$aDefaultPageIds;
    }
     ...
     *
     * @param  bool   $reload  Flag specifying to reload data from locales model
     * @param  string $locale  Locale, if not passed current data locale will be used
     * @return int  0 if not found
     */
    public static function getDefaultPageId($reload = FALSE, $locale = 'auto'){
        self::getDefaultPageIds();
        if('auto' == $locale){
            $locale = AMI_Registry::get('lang_data');
        }
        return
            isset(self::$aDefaultPageIds[$locale])
                ? self::$aDefaultPageIds[$locale]
                : 0;
    }
