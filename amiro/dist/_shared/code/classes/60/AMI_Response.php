 ...
 * @version    $Id$ 
 * @since      5.10.0 
 * @size       57107 xkqwymygzwkutpniisxgwmrgwznmmiwukwwkltykruwltlkuztmywksuiwiklqumtmwkpnir
 */
?><?php
...
class AMI_Response{
    ...
    */
    ...
    public function setBuffering($isBuffered, $force = FALSE){
        ...
        if($this->isBuffered !== (bool)$isBuffered){
            ...
            if($isBuffered){
                $this->HTTP->deleteHeader('X-Accel-Buffering: no');
                if(isset($conn)){
                    $conn->DelHeader('X-Accel-Buffering: no');
                }
            }else{
                while(ob_get_level()){
                ...
                $this->HTTP->addHeader('X-Accel-Buffering: no');
                if(isset($conn)){
                    $conn->AddHeader('X-Accel-Buffering: no');
                }
            }
    ...
    public function send(){
        ...
        if($this->type === 'HTML'){
            if(
                AMI_Service::isDebugVisible() &&
                !empty($GLOBALS['sys']['err']['extdeb'])
            ){
        ...
        if(
            !defined('AMI_60') &&
            FALSE !== mb_strpos($this->benchType, 'FAST')
        ){
...
final class AMI_HTTPResponse{
     ...
     */
    public function addHeader($header, $bReplace = true, $code = null){
        $this->aHeaders[$header] = array($bReplace, $code);
    ...
    public function deleteHeader($header){
        ...
        if(is_callable(array($conn, 'DelHeader'))){
            $conn->DelHeader($header);
        }
        unset($this->aHeaders[$header]);
    ...
    /**
     * Deletes  "200 Ok" HTTP header.
     *
     * @return AMI_HTTPResponse
     * @amidev
     * @since  7.0.0
     */
    public function deleteHeader200(){
        return $this
            ->deleteHeader('Status: 200 OK')
            ->deleteHeader($this->protocol . ' 200 OK');
    }
