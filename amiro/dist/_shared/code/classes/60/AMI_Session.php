 ...
 * @version    $Id$ 
 * @since      5.10.0 
 * @size       16248 xkqwxipszgugtuxgqtguuiqqggtxzsrnswltulytuswqqxrplnqrmqqixzgssrqxptlppnir
 */
?><?php
...
class AMI_Session{
    ...
    private function __construct(array $aOptions = array()){
        if(!empty($aOptions['locale'])){
            ...
            $this->locale = getLangFromURL(
                $oRequest->getURL('url'),
                $GLOBALS['ROOT_PATH_WWW']
            );
            if(
                empty($this->locale) &&
                AMI::issetAndTrueOption('core', 'allow_multi_lang')
            ){
                $this->locale = $oRequest->get('ami_locale', 'en');
            }
        }
        $this->side = AMI_Registry::get('side');
        if('adm' != $this->side){
            $this->oSession =
                isset($GLOBALS['oSession']) && is_object($GLOBALS['oSession'])
                ? $GLOBALS['oSession']
                : new CMS_Session($GLOBALS['cms'], $this->locale);
        }else{
            $this->oSession = admSession();
        }
    ...
    public function isStarted(){
        return
            'adm' != $this->side
            ? $this->oSession->IsSessionStarted()
            : TRUE;
    }
    ...
    public function login($userName, $password){
        ...
        if(($result !== false) && is_array($result) && $result['active'] == 1){
            ...
            $this->oSession->addJsSessionCookie('ami_efa', $oUser->ami_efa);
    ...
    public function loginAsUser($userName){
        $result = $this->getMember()->loginAsUser($this->oCMS, $userName, 'complex', FALSE);
        if(($result !== false) && is_array($result) && $result['active'] == 1){
            $oUser = $this->getUserData();
            $this->oSession->addJsSessionCookie('nickname_cookie', $oUser->nickname);
            $this->oSession->addJsSessionCookie('username_cookie', $oUser->username?$oUser->username:$oUser->login);
     ...
     *
     * @param  bool $asRawData  Flag specifiying to return raw session data,
     *                          not object (since 7.0.0)
     * @return AmiUsers_Users_TableItem|array|null
     */
    public function getUserData($asRawData = FALSE){
        if(!$this->isStarted()){
            $this->start();
        }
        /*
