 ...
 * @version    0.8 alpha 
 * @since      6.0.2 
 * @size       13578 xkqwntkyrkgipwryxtnzqunzqzlzklmgptsnrkrpnqxppqurlzprlktsslrlupnilswxpnir
 */
?><?php
...
class AMI_Storage_FS implements AMI_iStorage{
    ...
    public function save($file, $content, $asDefault = FALSE){
        ...
        if(FALSE !== $res){
            if(!@chmod($file, 0666)){
                trigger_error('File to change mode "' . $file . '"');
            }
        }
    ...
    public function copy($from, $to){
        ...
        if($res){
            if(!@chmod($to, 0666)){
                trigger_error('File to change mode "' . $to . '"');
            }
        }
    ...
    public function rename($from, $to){
        ...
        if($res){
            if(!@chmod($to, 0666)){
                trigger_error('Failed to change mode for "' . $to . '"');
            }
        }
