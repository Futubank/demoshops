 ...
 * @version    $Id$ 
 * @since      5.10.0 
 * @size       30964 xkqwxymrwpiqnxggtmgnzmmmnwswnymssxrlslyqmqwqkpzzwgmkxkqkynisuztslumppnir
 */
?><?php
...
interface AMI_iTemplate{
     ...
     * @since 7.0.0
     */
    const LOCAL_LNG_MOD_OPT_PATH = '_local/_admin/templates/lang/options';
...
class AMI_TemplateSystem implements AMI_iTemplate{
     ...
     * @param  string $innerContent  Content inside tag
     * @param  string $outerContent  Content betwen openinig and closing tags, no closing tag if blank
     * @param  bool   $replace       Flag specifying to replace existing tags with same name
     * @return void
     * @since  7.0.0
     */
    public function addHTMLHeaderTag($name, $innerContent, $outerContent = '', $replace = FALSE){
        $this->oTpl->addHeaderTag($name, $innerContent, $outerContent, !$replace);
    }

    /**
     * Appends CSS style to HTML HEAD.
     *
     * If passed URL has no domain, next parts will be prepended to URL:
     * * "http://domain/_mod_files/_css/" for front side,
     * * "http://domain/_local/_admin/_caa/" for admin side.
     *
     * Example:
     * <code>
     * // "_local/front_functions.php"
     ...
     * );
     * // will add code:
     * // <style type="text/css">h1 { color: maroon; margin-left: 40px; }</style>
     * </code>
     *
     * @param  string $url            Style full URL
     * @param  string $inlineContent  If != '' inline style will be added
     * @param  bool   $isSysContext   TRUE if internal system call
     * @return void
     * @since  7.0.0
     */
    public function addHTMLHeaderStyle($url, $inlineContent = '', $isSysContext = FALSE){
        if('' === $inlineContent){
            $this->oTpl->addStyle($url, array(), $isSysContext);
        }else{
            $this->addHTMLHeaderTag(
                'style',
                'type="text/css"',
                $inlineContent
            );
