 ...
 * @version    $Id$ 
 * @since      6.0.2 
 * @size       36986 xkqwlqquwpmlqxsuwixymxzigmgzgkttlgtkslpzusylmsmyxgiyrikugukyymztuqinpnir
 */
?><?php
...
class AMI_Tx_Cmd_Args{
    ...
    public function overwrite($name, $value){
        ...
        if(isset($this->aData[$name])){
            $oldValue =
                $name === 'mode'
                    ? '0x' . dechex($this->aData[$name])
                    : var_export($this->aData[$name], TRUE);
        }else{
            $oldValue = 'NULL';
        }
        $valueToLog =
        ...
        AMI_Tx::log(get_class($this) . '::$' . $name . ' (' . $oldValue . ') is overwritten by ' . $valueToLog /* . " from\n" . $e->getTraceAsString()*/, AMI_Service::LOG_WARN);
