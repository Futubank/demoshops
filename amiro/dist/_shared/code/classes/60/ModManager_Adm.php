 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       78696 xkqwzgxrgzqwwxiztgsriggmtqpwtlgwpxyltutxqplymigtpikwuzqztlizzwzimmtupnir
 */
?><?php
    ...
    public function init(){
        ...
        if($modId || $pkgHyperConfig){
            ...
            if(!$modId && $aCaptions){
                ...
                $this->addField(
                    array(
                        'name'       => $GLOBALS['Core']->isInstalled('sys_groups') ? 'add_to_groups' : 'edit_at_front',
                        'type'       => 'checkbox',
                        'attributes' => array('checked' => 'checked'),
                        'position' => 'advanced.end'
                    )
                );
            }
        }else{
    ...
    public function dispatchUninstall($name, array $aEvent, $handlerModId, $srcModId){
        ...
        if($oDeclarator->isRegistered($modId)){
            ...
            try{
                ...
                $GLOBALS['Core']->GetModule($modId)->setInstalled(FALSE);
