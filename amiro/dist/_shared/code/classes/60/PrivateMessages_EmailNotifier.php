 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       7165 xkqwllgxykuirilzgiritrylslgltxggymtlyqzxkxwmmxunsgxxzlmplytxrxulkxyppnir
 */
?><?php
    ...
    public static function send($messageId){
        ...
        if($oRecipient->email){
            ...
            if($oMessage->is_broadcast){
                ...
                // Shared hosting broadcast hack
                if('service' == $aScope['broadcast']){
                    $oTpl = AMI::getResource('env/template_sys');
                    $aLocale = $oTpl->parseLocale('_local/_admin/templates/lang/private_messages.lng');
                    $aLocale['body_host_broadcast_start'] = $aScope['message_body'] . $aLocale['body_host_broadcast_start'];
                    $oTpl->setBlockLocale('private_message', $aLocale);
                    unset($aLocale, $oTpl);
                }
            }
