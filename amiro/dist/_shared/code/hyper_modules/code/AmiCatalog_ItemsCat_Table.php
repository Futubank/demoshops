 ...
 * @version    $Id$ 
 * @since      6.0.2 
 * @size       21999 xkqwzpnpxpsipwxnllgulntgrpzgmwlrtugsryrsrwrquqiwlnmyissiitknztupzzxmpnir
 */
?><?php
...
class AmiCatalog_ItemsCat_TableItem extends Hyper_AmiCatalog_Cat_TableItem{
    ...
    public function __construct(AMI_ModTable $oTable, DB_Query $oQuery = null){
        ...
        foreach($aItemFields as $field){
            if(
                !is_array($field) &&
                (
                    mb_substr($field, 0, 13) == 'custom_field_' ||
                    mb_substr($field, 0, 13) == 'price_caption'
                )
            ){
    ...
    public function save(){
        ...
        if($aChangedFields){
            if(
                in_array('id_parent', $aChangedFields) &&
                $id == $this->aData['id_parent']
            ){
                throw new AMI_ModTableItemException(
                    "Parent Id cannot be equal to Id",
    ...
    protected function prepareFields(array &$aFields){
        return;###
        static $needAddSlashes, $aHTMLFields, $aExclusions = array('header', 'sm_data');
