 ...
 * @version    $Id$ 
 * @size       34385 xkqwgzmguqzlsixwkxuxgzrksxykyqnwkpmnzgrgxunqwmynzxzymqxmnrxttiyrwwwkpnir
 */
?><?php
 ...
 * Fields description:
 * - <b>flags</b> - special flags bit mask (int);
...
class AmiCatalog_Items_TableItem extends Hyper_AmiCatalog_TableItem{
    ...
    public function __construct(AMI_ModTable $oTable, DB_Query $oQuery = null){
        ...
        $this->setFieldCallback('special_announce', array($this, 'fcbEncodedPercent'));
        ...
        $modId = $this->getModId();
        $aAvailableFields = array_filter(
            $this->oTable->getAvailableFields(),
            array($this, 'cbFilterOriginFields')
        );
        $aOriginFields = array_diff($aAvailableFields, $this->aLiteFields);
    ...
    public function save(){
        ...
        if($aChangedFields){
            ...
            $id = $this->getId();
            AMI_Registry::push('AMI/Module/Model/skipHTMLDecoding', TRUE);
            $this->runInPrevEnv(empty($id) ? 'add' : 'apply');
            AMI_Registry::pop('AMI/Module/Model/skipHTMLDecoding');
    ...
    protected function runInPrevEnv($action){
        ...
        */
        ...
        if(isset($this->aData['ext_img'])){
            $this->aData['picture'] = $this->aData['ext_img'];
            $this->aData['popup_picture'] = $this->aData['ext_img_popup'];
            $this->aData['small_picture'] = $this->aData['ext_img_small'];
        }
        if($action == 'apply'){
            ...
            if(!empty($this->aOrigData['html_meta'])){
                foreach($this->aOrigData['html_meta'] as $field => $value){
                    ...
                    $this->oModule->Engine->cms->VarsPost[$field] = addslashes($value);
                }
                // $this->oModule->Engine->cms->Vars['is_keywords_manual'] = $this->aData['html_is_kw_manual'];
                $this->oModule->Engine->cms->VarsPost['is_keywords_manual'] = addslashes($this->aData['html_is_kw_manual']);
            }
            if(isset($this->aOrigData['aData']['sublink'])){
                // $this->oModule->Engine->cms->Vars['original_sublink'] = $this->aOrigData['aData']['sublink'];
                $this->oModule->Engine->cms->VarsPost['original_sublink'] = addslashes($this->aOrigData['aData']['sublink']);
            }
        }
        ...
        foreach($this->aData as $field => $value){
            ...
            // $this->oModule->Engine->cms->Vars[$field] = $value;
            if(in_array($field, array('html_keywords', 'html_description'))){
                $value = htmlspecialchars(str_replace('"', '', $value));
            }
            $this->oModule->Engine->cms->VarsPost[$field] = $value; ### addslashes($value);
        }
        $this->prepareFields($this->oModule->Engine->cms->VarsPost); ###
    ...
    protected function prepareFields(array &$aFields){
        static
            $needAddSlashes, $aHTMLFields,
            $aExclusions = array('html_keywords', 'html_description');

        if(!isset($needAddSlashes)){
            $needAddSlashes = TRUE; ###$GLOBALS['AMI_ESCAPE_REQUEST'];
            $aHTMLFields = AMI::getProperty('core', 'html_area_fields');
        }

        foreach(array_keys($aFields) as $key){
            if(in_array($key, $aExclusions)){
                continue;
            }
            $value = $aFields[$key];
            if(is_array($value)){
                // $value = $this->prepareFields($aFields[$key]);
            }else{
                if(in_array($key, $aHTMLFields)){
                    $value = str_replace('%%', '&#037;&#037;', $value);
                }elseif(!AMI_Skin::isModuleSpecialMode($this->getModId())){###
