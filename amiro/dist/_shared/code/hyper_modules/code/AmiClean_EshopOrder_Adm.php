 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       111194 xkqwnzqgyqkykiipzkrzmisllmlptqzgsmzsyqkzlnkytmnqsmgqpgwrrpixwpgxttlppnir
 */
?><?php
    ...
    public function __construct(){
        ...
        */
        ...
        $aLocale = $oTpl->parseLocale(
            '_local/_admin/templates/lang/eshop_order.lng',
        ...
        $oCol = AMI::getSingleton('db')->fetchCol($sql);
        foreach($oCol as $driver){
            if('demo' == $driver){
                continue;
            }
            if(
                isset($GLOBALS['BILLING_DRIVERS_ALIASES'][$driver]) &&
                !empty($aLocale[$GLOBALS['BILLING_DRIVERS_ALIASES'][$driver]])
            ){

                $caption = $aLocale[$GLOBALS['BILLING_DRIVERS_ALIASES'][$driver]];
            }elseif(isset($aLocale[mb_strtoupper($driver) . '_DRIVER'])){
                $caption = $aLocale[mb_strtoupper($driver) . '_DRIVER'];
            }elseif(isset($aLocale[$driver])){
                $caption = $aLocale[$driver];
            }else{
                $caption = $driver;
            }
            $aData[] = array(
        ...
        $aData = array();
        $oList =
            AMI::getResourceModel('eshop_shipping_methods/table')
            ->getList()
            ->addColumn('header')
            ->addWhereDef('AND `hidden` != 1 AND `id_parent` = 0')
            ->load();
...
class AmiClean_EshopOrder_FormAdm extends Hyper_AmiClean_FormAdm{
    ...
    protected function _getView($resIdTail, $bAddResId = TRUE){
        $oView = parent::_getView($resIdTail, $bAddResId);
    ...
    public function handleBeforeSetValuesModelItem($name, array $aEvent, $handlerModId, $srcModId){
        ...
        // Order changes
        $aCollectFields =
            AMI_Skin::isModuleSpecialMode($this->getModId())
                ? array('status', 'adm_comments')
                :  array('status', 'adm_comments', 'login', 'email', 'firstname', 'lastname', 'company', 'comments', 'tax', 'shipping', 'total');
            ...
            if(count($this->aItemsChanges)){
                ...
                foreach($this->aItemsChanges as $name => $aItem){
                    if(isset($aOrderItems[$aItem['item_id']])){
                        ...
                        if(isset($aItem['new']['price'])){
                            $oItem->price = $aItem['new']['price'];
                        }
...
#        parent::init()
    ...
    public function addFields(){
        ...
        if($oOrder->id){
            ...
            $this->addField(array('name' => 'name_static', 'type' => 'static', 'value' => $oOrder->name));
            ...
            $this->addField(array(
                ...
                'type'     => 'static',
                'value'    => $oOrder->id,
                'position' => !AMI_Skin::isModuleSpecialMode($this->getModId()) ? 'statustab.begin' : ''
            ));
            ...
            $this->addField(array('name' => 'date_created', 'type' => 'static', 'value' => $date));
            ...
            $this->addField(array('name' => 'login_static', 'type' => 'static', 'value' => $login));
            ...
            if(is_array($oOrder->status_history)){
                $aStatHistory = $oOrder->status_history;
                list($date, ) = each($aStatHistory);
                $this->addField(array(
                    'name'  => 'status_changed',
                    'type'  => 'static',
                    'value' => AMI_Lib_Date::formatUnixTime($date, AMI_Lib_Date::FMT_BOTH)
                ));
            }
            if('' !== $oOrder->comments){
                $this->addField(array(
                    'name'     => 'comments_static',
                    'type'     => 'static',
                    'value'    => $oOrder->comments
                ));
            }
            $aField = array(
                'name'             => 'adm_comments',
                ...
                'rows'             => 3
                // ,
                // 'skipHTMLEncoding' => AMI_Skin::isModuleSpecialMode($this->getModId())
            );
            ...
            $aCustInfo =
                (
                    is_array($oOrder->custom_info)
                        ? $oOrder->custom_info
                        : array()
                ) + array(
                    'get_type_name'      => NULL,
                    'shipping_conflicts' => NULL,
                    'contact'            => NULL
                );
            ...
            $aOrderData = $oOrder->getData() + array(
                'members_adm_link' => 'eshop_user.php?',
                'email_encoded'    => str_replace(' ', '%20', $oOrder->email)
            );
            $aCustomInfo = array(
                'name'        =>
                    $aSysInfo['person_type'] != 'natural'
                        ? $oOrder->company
                        : trim($oOrder->firstname . ' ' . $oOrder->lastname),
                'email'       => $oOrder->email,
                'person_type' => $aSysInfo['person_type'],
                'contact'     => $aCustInfo['contact'], // phone
                'login'       => $oOrder->login,
                'ip'          => $aSysInfo['ip']
            );
            foreach($aCustomInfo as $name => $value){
                if(is_null($value)){
                    continue;
                }
                $custInfo .= $this->getComplexInfoRow(
                    $name,
                    isset($this->aLocale[$value]) ? $this->aLocale[$value] : $value,
                    $aOrderData
                );
            }
            ...
            if(is_array($oOrder->custom_info)){
                if(array_key_exists('country', $oOrder->custom_info)){
                    $aCountries = $oTpl->parseLocale(
                        '_local/_admin/templates/lang/country.lng',
                        AMI_Registry::get('lang_data')
                    );
                }

                $customerAddress =
                    'natural' != $aCustomInfo['person_type']
                        ? $oOrder->custom_info['address']
                        : $oEshopOrder->getCustomerAddress($oOrder->custom_info);
                ...
                foreach($oOrder->custom_info as $name => $val){
                    if(array_key_exists($name, $aCustomInfo)){
                        continue;
                    }
                    if(
                        $name == 'id_coupon' &&
                        AMI::isModInstalled($oOrder->custom_info['coupon_owner'] . '_coupons')
                    ){
                        // @todo: remove hardcode, create models for coupons
                        // $this->oEshop->initByOwnerName($aCustInfo['coupon_owner']);
                        $snippet =
                            "SELECT `co`.`coupon`, `coc`.`name` `coupon_category` " .
                            "FROM `%s` `co` " .
                            "INNER JOIN `%s` `coc` " .
                            "ON `co`.`id_cat` = `coc`.`id` " .
                            "WHERE `co`.`id` = %s";
                        ...
                        if(sizeof($aResult) == 2){ // coupon and coupon_category
                            ...
                            $this->addField(array(
                                ...
                                'value'    => $coupon,
                                'position' =>
                                    $this->isPlaceholderExists('shipping')
                                        ? 'shipping.before'
                                        : ''
                            ));
                        }
                    ...
                    if(!empty($this->aLocale[$name])){
                        ...
                        if('' !== $val){
                            $deliveryInfo .= $this->getComplexInfoRow(
                                $name,
                                nl2br($val),
                                $aOrderData
                            );
            ...
            if('' != $oOrder->tracking_number){
                $deliveryInfo .= $this->getComplexInfoRow(
                    'form_field_tracking_number',
                    $oOrder->tracking_number,
                    array('url' => preg_match('/^https?:\/\//i', $oOrder->tracking_number))
                );
            }
            /*
            if('' !== $oOrder->comments){
            ...
            */
            ...
            // $this->addField(array('name' => 'goods_value', 'type' => 'static', 'value' => $total));

            $paymentInfo .= $this->getComplexInfoRow(
                'caption_goods_value',
                $total
            );
            ...
            if(is_array($oOrder->system_info) && isset($oOrder->system_info['fee_percent'])){
                $paymentMethodFee = $oEshopOrder->getTotal($oOrder, 'payment_method_fee', AMI_EshopOrder::FMT_MONEY);
                // $this->addField(array('name' => 'payment_method_fee_static', 'type' => 'static', 'position' => 'amount.after', 'value' => $paymentMethodFee));
                $paymentInfo .= $this->getComplexInfoRow(
                    'payment_method_fee',
                    $paymentMethodFee
                );

                $driver = $oOrder->system_info['driver'];

                if(
                    isset($GLOBALS['BILLING_DRIVERS_ALIASES'][$driver]) &&
                    !empty($this->aLocale[$GLOBALS['BILLING_DRIVERS_ALIASES'][$driver]])
                ){
                    $driver = $this->aLocale[$GLOBALS['BILLING_DRIVERS_ALIASES'][$driver]];
                }elseif(isset($this->aLocale[mb_strtoupper($driver) . '_DRIVER'])){
                    $driver = $this->aLocale[mb_strtoupper($driver) . '_DRIVER'];
                }elseif(isset($this->aLocale[$driver])){
                    $driver = $this->aLocale[$driver];
                }
                if($driver){
                    // $this->addField(array(
                    //     'name'     => 'payment_method',
                    //     'type'     => 'static',
                    //     'position' => 'payment_method_fee_static.after',
                    //     'value'    => $driver
                    // ));
                    $paymentInfo .= $this->getComplexInfoRow(
                        'caption_payment_method',
                        $driver
                    );
                }
            }

            // $this->addField(array('name' => 'comments_static', 'type' => 'static', 'value' => $oOrder->comments));
            ...
            */
            ...
            if(isset($aOrderItems[$oOrder->id])){
                ...
                foreach($aOrderItems[$oOrder->id] as $oItem){
                    ...
                    if(is_array($aProduct)){
                        ...
                        if($aProduct["price".$priceNumber] != ""){
                            ...
                            $aPrice = $oEshop->calcPrice(
                                ...
                                $priceNumber ? $aProduct["db_currency" . $priceNumber] : '',
                                true,
                                array(
                                    "qty"         => $oItem->qty,
                    ...
                    $aScope = array(
                        'product_url'                   =>
                            'engine.php?mod_id=eshop_item&action=edit&id=' .
                            $aProduct['id'] . '&scroll_to_form=1',
                        'number'                        => $i,
                        'style'                         => $style,
            ...
            if(is_array($aStatHistory)){
                ...
                $aStatHistory = array_reverse($aStatHistory, TRUE);
            ...
            if(!AMI_Skin::isModuleSpecialMode($this->getModId())){
                 ...
                 * ITEMS TAB
                 */
                $aCustomInfo = is_array($oOrder->custom_info) ? $oOrder->custom_info : '';
                // Hack to avoid creating spare lang vars
                foreach(array_keys($aCustomInfo) as $name){
                ...
                $this->addField(array('name' => 'comments', 'type' => 'textarea', 'position' => 'itemstab.begin'));

                $this->addField(array(
                    'name'   => 'sep_customer_info',
                    'type'   => 'static',
                    'marker' => 'separator'
                ));
                ...
                $name = 'contact';
                if(isset($aCustomInfo[$name])){
                    $this->addField(array(
                        'name'  => 'ci_' . $name,
                        'value' => $aCustomInfo[$name]
                    ));
                    unset($aCustomInfo[$name]);
                }
                $this->addField(array('name' => 'login'));
                ...
                if(isset($aCustomInfo[$name])){
                    if(
                        'show_shipping_for_each_type' == $aCustomInfo['shipping_conflicts'] ||
                        !sizeof($aShippingSelect)
                    ){
                        $this->addField(array(
                            'name'  => 'ci_' . $name,
                            'type'  => 'static',
                        ...
                        $this->addField(array(
                            'name'  => 'ci_' . $name,
                            'type'  => 'select',
                            'data'  => $aShippingSelect,
                            'value' => $aCustomInfo[$name]
                        ));
                    }
                    unset($aCustomInfo[$name], $aCustomInfo['shipping_conflicts']);
                }
                foreach($aCustomInfo as $name => $val){
                    if($name == 'country' && isset($aCountries[$val])){
                        $val = $aCountries[$val];
                    }
                    $this->addField(array('name' => 'ci_' . $name, 'value' => $val));
                }

                $this->addField(array('name' => 'tracking_number'));
                // $this->addField(array('name' => 'adm_comments_static', 'type' => 'static', 'position' => 'comments.after', 'value' => $oOrder->adm_comments));

                /*
                // Payment tax
                if(is_array($oOrder->system_info) && isset($oOrder->system_info['fee_percent'])){
                    $paymentMethodFee = $oEshopOrder->getTotal($oOrder, 'payment_method_fee', AMI_EshopOrder::FMT_NONE);
                    $this->addField(array('name' => 'payment_method_fee', 'position' => 'adm_comments_static.after', 'value' => $paymentMethodFee));
                }
                */
                /*
                Not needed.
                ...
                $this->addSection('refresh_currency'); // adm_comments_static.after
                ...
                */
                ...
                $aScope = array(
                    ...
                    'item_currency'       => $oOrder->data['currency'][$oOrder->data['base_currency']['code']]['name'],
                    'order_base_currency' => $oOrder->data['base_currency']['code']
                );
                $productsEdit = $oTpl->parse($this->tplBlockName . ':products_edit_table', $aScope);
                $this->addField(array('name' => 'products_edit_table', 'type' => 'static', 'html' => $productsEdit, 'skipHTMLEncoding' => true));
                ...
                foreach($oAuditList as $oAuditItem){
                    ...
                    if(count($oAuditItem->data['items'])){
                        foreach($oAuditItem->data['items'] as $name => $aItem){
                            ...
                            if(!empty($aItem['new']['item_id']) && empty($aItem['prev']['item_id'])){
                ...
                $this->addField(array('name' => 'order_changes_table', 'type' => 'static', 'html' => $orderChanges, 'skipHTMLEncoding' => true));
            }

            $this->addSection('notify', 'status_changed.after');
        }
    ...
    /**
     * Parses 'comlex_info_row' set and returns result.
     *
     ...
     * @return string
     */
    protected function getComplexInfoRow($name, $value, array $aScope = array()){
        return $this->getTemplate()->parse(
            $this->tplBlockName . ':comlex_info_row',
            array(
                'name'    => $name,
                'value'   => $value,
                'caption' => $this->aLocale[$name]
            ) + $aScope
        );
    }
...
class AmiClean_EshopOrder_ListViewAdm extends AMI_Module_ListViewAdm{
    ...
    protected $aPlaceholders = array(
        ...
        '#flags', 'flags',
        '#columns',
            'id', 'u_fullname', 'email', 'name', 'date_created', 'status', 'amount', 'payment_method',
        'columns',
        '#actions', 'actions',
        'list_header'
    );
    ...
    public function init(){
        ...
        $this
            ...
            ->addColumn('payment_method', 'float')
            ...
            ->addSortColumns(array('id', 'u_fullname', 'email', 'name', 'status', 'amount'));

        $this->formatColumn('u_fullname', array($this, 'fmtFullname'));
        $this->formatColumn('email', array($this, 'fmtEMail'));
        ...
        $this->formatColumn('payment_method', array($this, 'fmtPaymentMethod'));
     ...
     * @see    AMI_ModListView::handleFormatCell()
     */
    protected function fmtEMail($value, array $aArgs){
        $aCustomInfo = $aArgs['oItem']->custom_info;
        if(!empty($aCustomInfo['contact'])){
            $value .= '<br />' . $aCustomInfo['contact'];
        }
    ...
    /**
     * Payment method column formatter.
     *
     * @param  string $value  Value to format
     * @param  array  $aArgs  Arguments
     * @return mixed
     * @see    AMI_ModListView::handleFormatCell()
     */
    protected function fmtPaymentMethod($value, array $aArgs){
        $oItem = $aArgs['oItem'];
        $driver = $oItem->system_info['driver'];
        ...
        if(
            ...
            return $this->aLocale[$GLOBALS['BILLING_DRIVERS_ALIASES'][$driver]];
        }elseif(isset($this->aLocale[mb_strtoupper($driver) . '_DRIVER'])){

            return $this->aLocale[mb_strtoupper($driver) . '_DRIVER'];
        }elseif(isset($this->aLocale[$driver])){
            return $this->aLocale[$driver];
        }
