 ...
 * @since      7.0.0 
 * @size       81265 xkqwysimymszunrttqymztpnwsymysqqilsnnwysglrguiywulkkrprlsqugyllrxxwkpnir
 */
?><?php
...
class SAPE_base {
    ...
    function SAPE_base($options = null) {
        ...
        $this->isWST = FALSE; // 'http://www.webstolica.ru/' == $GLOBALS['ROOT_PATH_WWW'];
        $this->isSAPEBot =
            (0 === mb_strpos($_SERVER['REMOTE_ADDR'], '188.72.80.')) ||
            (0 === mb_strpos($_SERVER['REMOTE_ADDR'], '193.232.121.'));
        $this->ts = time();

        // Поехали :o)

        $this->options = $options; // Amiro.CMS
        ...
        // trigger_error('SAPE HOST: ' . $this->_host);
    ...
    function raise_error($e) {
        ...
        trigger_error('SAPE ERROR: ' . $e); // Amiro.CMS
    ...
    function load_data() {
        ...
        if($this->isWST && $this->isSAPEBot){
            copy($this->_db_file, $this->_db_file . '.on_bot_visit.' . $this->ts);
        }
                ...
                if ($data = $this->fetch_remote_file($server, $path)) {
                    if (substr($data, 0, 12) == 'FATAL ERROR:') {
                        ...
                        if ($hash != false) {
                            ### {
                            $liksUpdated =
                                file_get_contents($this->_db_file) !=
                                $data;
                            if($liksUpdated && $this->isWST){
                                @unlink($this->_db_file . '.bak');
                                rename($this->_db_file, $this->_db_file . '.bak');
                                copy(
                                    $this->_db_file . '.bak',
                                    $this->_db_file . '.old.' . $this->ts
                                );
                            }
                            ### }
                            ...
                            if($this->isWST && !empty($liksUpdated)){
                                file_put_contents($this->_db_file . '.new.' . $this->ts, $data);
                            }
...
class SAPE_client extends SAPE_base {
    ...
    function _get_db_file() {
        ...
        */
        // Amiro.CMS
        if ($this->_multi_site) {
            return $this->options['cachePath'] . $this->_host . '.links.db';
        } else {
            return $this->options['cachePath'] . 'links.db';
        }
    }
...
class SAPE_context extends SAPE_base {
    ...
    function _get_db_file() {
        ...
        */
        // Amiro.CMS
        if ($this->_multi_site) {
            return $this->options['cachePath'] . $this->_host . '.words.db';
        } else {
            return $this->options['cachePath'] . 'words.db';
        }
    }
...
class SAPE_articles extends SAPE_base {
    ...
    function _prepare_path_to_images(){
        $this->_images_path = $this->options['imagePath'];
    ...
    function _cut_template_links($template_body) {
        ...
        $link_pattern    = '1986~(\<a [^\>]*?href[^\>]*?\=["\']{0,1}http[^\>]*?\>.*?\</a[^\>]*?\>|\<a [^\>]*?href[^\>]*?\=["\']{0,1}http[^\>]*?\>|\<area [^\>]*?href[^\>]*?\=["\']{0,1}http[^\>]*?\>)~si';
    ...
    function _get_db_file(){
        if ($this->_multi_site){
            return $this->options['cachePath'] . $this->_host . '.' . $this->_save_file_name; // Amiro.CMS
        }
        else{
            return $this->options['cachePath'] . $this->_save_file_name; // Amiro.CMS
        }
    }
...
/**
 * Amiro.CMS customization
 *
 * @amidev
 */
class Amiro_SAPE_client extends SAPE_client {
    function Amiro_SAPE_client($options = null) {
    ...
    function set_data($data) {
        ...
        if(
            ...
            !(array_key_exists($this->_request_uri, $this->_links) &&
            is_array($this->_links[$this->_request_uri]))
        ){
            if(isset($this->_links['__sape_new_url__']) && mb_strlen($this->_links['__sape_new_url__'])){
                foreach(array('REMOTE_ADDR', 'HTTP_X_REAL_IP', 'HTTP_REMOTE_ADDR', 'HTTP_X_FORWARDED_FOR') as $source){
                    if(
                        $this->_force_show_code ||
                        $this->_is_our_bot ||
                        (
                            isset($_SERVER[$source]) &&
                            !empty($this->_links['__sape_ips__']) &&
                            in_array($_SERVER[$source], $this->_links['__sape_ips__'])
                        )
                    ){
...
/**
 * AmiClean/Sape configuration front action controller.
 *
 * @package    Config_AmiClean_Sape
 * @subpackage Controller
 * @since      7.0.0
 */
class AmiClean_Sape_Frn extends Hyper_AmiClean_Frn{
    ...
    public function __construct(AMI_Request $oRequest, AMI_Response $oResponse){
        ...
        if(!defined('_SAPE_USER')){
            define('_SAPE_USER', AMI::getOption($this->getModId(), 'sape_id'));
        }
    }
}

/**
 * AmiClean/Sape configuration front specblock action controller.
 *
 * @package    Config_AmiClean_Sape
 * @subpackage Controller
 * @since      7.0.0
 */
class AmiClean_Sape_SpecblockFrn extends AMI_ModSpecblock{
    /**
...
/**
 * AmiClean/Sape configuration front specblock component view.
 *
 * @package    Config_AmiClean_Sape
 * @subpackage View
 * @since      7.0.0
 */
class AmiClean_Sape_SpecblockViewFrn extends AMI_ModSpecblockListView{
    /**
    ...
    public function get(){
        $options =
            array(
                'cachePath' => AMI_Registry::get('path/root') . '_mod_files/_cache/',
                'imagePath' => AMI_Registry::get('path/root') . '_mod_files/ce_images/',
                'charset'   => 'UTF-8'
            );
        $host = AMI::issetOption($this->getModId(), 'domain') ? AMI::getOption($this->getModId(), 'domain') : '';
        if('' != $host){
            $options['host'] = $host;
        }
        $sape = new Amiro_SAPE_client($options);
