 ...
 * @size       15907 xkqwnqmlylkgplylyqizklwtgwzizsqisntrylignsuwgznulxyzysgggugtmygptgpgpnir
 */
?><?php
...
class AmiClean_Webservice_FormViewAdm extends Hyper_AmiClean_FormViewAdm{
    ...
    public function init(){
        ...
        AMI_Lib_Array::sortMultiArray($aData, 'name');
    ...
    private function getRegisteredModules(){
        ...
        if($GLOBALS['Core']->IsInstalled('eshop_data_exchange')){
            $aRegistered[] = 'eshop_data_exchange';
        }
        foreach ($aRegistered as $index => $modId){
            if('eshop_data_exchange' == $modId){
                $hyper = 'ami_catalog';
            }else{
            ...
            if(in_array($hyper, array('ami_clean', 'ami_multifeeds', 'ami_multifeeds5', 'ami_users', 'ami_catalog'))){
                if(
                    ...
                    ((strpos($modId, 'eshop_') !== 0) || ($modId == 'eshop_item') || ($modId == 'eshop_order') || ($modId == 'eshop_data_exchange')) &&
