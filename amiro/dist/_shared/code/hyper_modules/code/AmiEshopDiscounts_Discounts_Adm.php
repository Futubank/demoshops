 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       20660 xkqwwxmyqpgsgnsyklirkxznnluytrysqxkguzwszunztwnmrlzqnxuiyqnxrttgyxuppnir
 */
?><?php
...
class AmiEshopDiscounts_Discounts_FilterModelAdm extends Hyper_AmiEshopDiscounts_FilterModelAdm{
    ...
    public function __construct(){
        ...
        if(!$isCategoryPopup){
            if($isPopup){
                $aConditions[] = array('caption' => 'all', 'value' => '');
            }else{
                $aConditions[] = array('caption' => 'condition_global', 'value' => 'global');
            }
        }
        $aCondition = array(
            'caption'  => 'condition_category',
            'value'    => 'category'
        );
        if($isPopup && !$isCategoryPopup && !$oRequest->get('condition', FALSE)){
            $aCondition['selected'] = TRUE;
        }
        $aConditions[] = $aCondition;
        ...
        if($isCategoryPopup){
            $aConditionField['flt_default'] = 'category';
        }
        if(!$isPopup){
