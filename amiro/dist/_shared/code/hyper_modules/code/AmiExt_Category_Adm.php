 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       8510 xkqwzrnuxtyzwupsxyxktunswpiliwwznwmqkguwtyiwyssyyurtninxmqzirygstuiypnir
 */
?><?php
...
class AmiExt_Category_ViewAdm extends AMI_ExtView{
    ...
    public function handleFormFields($name, array $aEvent, $handlerModId, $srcModId){
        ...
        if(
            !$itemId && AMI_Skin::isModuleSpecialMode($this->oExt->getModId())
        ){
            $itemId = (int)AMI::getSingleton('env/request')->get('category_id', 0);
        }
        ...
        if(
            !AMI_Skin::isModuleSpecialMode($this->oExt->getModId()) &&
        ...
        if(!AMI_Skin::isModuleSpecialMode($this->oExt->getModId())){
            $oFormView
