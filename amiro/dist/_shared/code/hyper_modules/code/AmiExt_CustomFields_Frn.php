 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       16075 xkqwxrggrtnxiqmilmxrkryrwiqwlgpgmxsiytglwxputmwwplrsgspkgruzgzurxlumpnir
 */
?><?php
...
class AmiExt_CustomFields_Frn extends AmiExt_CustomFields_Common{
    ...
    public function handlePreInit($name, array $aEvent, $handlerModId, $srcModId){
        ...
        if($oView){
            ...
            AMI_Event::addHandler('on_before_view_small', array($oView, 'handleBeforeView'), $modId);
        }
...
class AmiExt_CustomFields_ViewFrn extends AMI_ExtView{
    ...
    public function handleDetails($name, array $aEvent, $handlerModId, $srcModId){
        ...
        $isSubitem = isset($aEvent['type']) && $aEvent['type'] == 'subitems';

        if(!$isSubitem && ($name != 'on_item_details')){
            if($this->viewBodyType == 'cats' || $this->viewBodyType == 'sticky_cats'){
                $handlerModId = $handlerModId . '_cat';
                $aCustomFields = $this->oExt->getCategoryCustomFields();
            }elseif(
                empty($aEvent['cf_cat_scope_filled']) &&
                AMI_Registry::get('page/catId', 0) > 0
            ){
                // Fill current category custom fields
                $aEvent['cf_cat_scope_filled'] = TRUE;
                $aEvent['cf_name_prefix'] = 'cat_';
                $tmpViewBodyType = $this->viewBodyType;
                $this->viewBodyType = 'cats';
                AMI_Registry::push('AMI/Module/Environment/bodyType', 'cats');
                $aEvent = $this->handleDetails($name, $aEvent, $handlerModId, $srcModId);
                AMI_Registry::pop('AMI/Module/Environment/bodyType');
                $this->viewBodyType = $tmpViewBodyType;
                unset($tmpViewBodyType);
            }
        }
        ...
        $commonCustomFields = '';
        $namePrefix = isset($aEvent['cf_name_prefix']) ? $aEvent['cf_name_prefix'] : '';
        $oTpl = $this->getTemplate();
        if(
            in_array($bodyType, array('details', 'items', 'cats')) &&
            sizeof($this->oExt->aCrossHandlerData[$handlerModId]['db_fields'])
        ){
            ...
            foreach($this->oExt->aCrossHandlerData[$handlerModId]['db_fields'] as $fieldId => $field){
                ...
                if(
                    ($this->viewBodyType == 'sticky_items' || $this->viewBodyType == 'sticky_cats') &&
                    FALSE === mb_strpos($aCustomFields[$fieldId]['show_body_type'], ';body_urgent_items;')
                ){
                    continue;
                }
                if(
                    ($this->viewBodyType == 'items' || $this->viewBodyType == 'cats') &&
                    FALSE === mb_strpos($aCustomFields[$fieldId]['show_body_type'], ';body_items;')
                ){
                    continue;
                }
                if(
                ...
                if($isCommonList){
                    ...
                    $name = AmiExt_CustomFields_Common::PREFIX . $namePrefix . $aField['system_name'];
                    $aEvent['oItem']->setValue($name, $fieldHTML);
                    if(!empty($aEvent['cf_cat_scope_filled'])){
                        $oTpl->addGlobalVars(array($name => $fieldHTML));
                    }
                }
            }
        }
        if(mb_strlen($commonCustomFields)){
            ...
            unset($aSetData['db_fields'], $aSetData['common_db_fields']);
            $list = $this->parse('common_cf_list', $aSetData);
            $aEvent['aData']['common_cf_' . $namePrefix . 'list'] = $list;
            if(!empty($aEvent['cf_cat_scope_filled'])){
                $oTpl->addGlobalVars(array(
                    'common_cf_' . $namePrefix . 'list' => $list
                ));
            }
        }
