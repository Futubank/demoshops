 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       24526 xkqwnurtylkpqxykrpqrswilwtnyxsygntqlpupwssikyzrslilkpgturllsuwmlpwnmpnir
 */
?><?php
...
class AmiExt_Image_Frn extends AmiExt_Image{
    ...
    public function handleSaveModelItem($name, array $aEvent, $handlerModId, $srcModId){
        if($handlerModId === $this->getModId()){
            $aImages = array(
                'ext_img'       => $aEvent['aData']["ext_img"],
                'ext_img_popup' => $aEvent['aData']["ext_img_popup"],
                'ext_img_small' => $aEvent['aData']["ext_img_small"]
            );
            $root = AMI_Registry::get('path/root');
            $changed = FALSE;
            foreach($aImages as $type => $path){
                if(strpos($path, '_upload/') > 0){
                    $newPath = str_replace('_upload/tmp', 'ce_images', $path);
                    AMI_Lib_FS::copyFile($root . $path, $root . $newPath);
                    $aEvent['aData'][$type] = $newPath;
                    $aEvent['oItem']->{$type} = $aEvent['aData'][$type];
                    $changed = TRUE;
                }
            }
            if($changed){
                $aFilesToDel = array('_index_file.dat_', '_index_img_file.dat_');
                foreach($aFilesToDel as $file){
                    $fullPath = $root . '_mod_files/ce_images/' . $file;
                    if(file_exists($fullPath)){
                        AMI_Lib_FS::deleteFile($fullPath);
                    }
                }
            }
        }
    ...
    public function handleSetData($name, array $aEvent, $handlerModId, $srcModId){
        ...
        if(!$this->bodyType && !AMI_Skin::isModuleSpecialMode($this->getModId())){
            ...
            if($this->genLinkToPic($curModule, $preffx, $origFilePath, $origFileLevel, isset($aItem[$field]) ? $aItem[$field] : null, $itemId, $vRes, FALSE)){
                $aPicData = array_merge(
                    ...
                    array(
                        ...
                        'picture'   => $vRes["src"],
                        'src'       => $vRes["src"],
                        'title'     => str_replace('"', '&quot;', $aItem['header']),
                        'alt'       => str_replace('"', '&quot;', $aItem['header']),
                ...
                $aItem[$preffx."picture_title"] = str_replace('"', '&quot;', $aItem['header']);
