 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       25089 xkqwmtlrmntpkwzyuqnirxxwznknlnsupuzqqnmxktiuwuurwsquyppxslmyznglyrtrpnir
 */
?><?php
...
class AmiExt_Rating_Service{
    ...
    public static function isAlreadyRated($id = 0){
        ...
        $sql =
            "SELECT `id` " .
            "FROM `cms_rate_history` " .
            "WHERE " .
                "`id_module` = %s AND " .
                "`id_item` = %s AND " .
                "(" .
                    ($aSettings['userLoggedIn'] ? "`id_user`" : "`ip`") . " = %s OR " .
                    "`vid` = %s" .
                ") AND " .
                ($aSettings['userLoggedIn'] ? "`date_expire_user`" : "`date_expire_ip`") . " > %s";

        $oQuery = DB_Query::getSnippet($sql)
            ->q($aSettings["modId"])
            ->q(intval($id))
