 ...
 * @version    $Id$ 
 * @since      x.x.x 
 * @size       27408 xkqwuipinnstmwuukrlzriikwstmxkmukpypyyxmmuwguusiptrqzxlunpyqsgqsnxqqpnir
 */
?><?php
...
class AmiExt_Rss_ViewFrn extends AMI_ExtView{
    ...
    public function handleRSSLinksSmall($name, array $aEvent, $handlerModId, $srcModId){
        ...
        $omitLinkRSS = false;
        $option =
            AMI::issetAndTrueOption($modId, 'use_categories') ? 'spec_cat_id_pages' : 'spec_id_pages';
        if(AMI::issetOption($modId, $option)){
        ...
        if(
            !$omitLinkRSS &&
            AMI::issetOption($modId, 'rss_display_page') &&
            AMI::issetOption($modId, 'rss_display_image')
        ){
            $url = '';
            $catId = AMI_Registry::get('page/catId', 0);
            if($catId && AMI_Registry::get('page/modId', '') == $modId){
                if(AMI_ModDeclarator::getInstance()->isRegistered($modId . '_cat')){
                    $oItem = AMI::getResourceModel($modId . '_cat/table')->find($catId);
                    $url = $oItem->getURL();
                }
            ...
            $pageId = 0;
            if(isset($aSpecIdPages)){
                reset($aSpecIdPages);
                list($pageId) = each($aSpecIdPages);
            }
            $this->linkRSS =
                AMI_Registry::get('path/www_root') .
