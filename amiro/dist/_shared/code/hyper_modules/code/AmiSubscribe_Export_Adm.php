 ...
 * @version    $Id$ 
 * @size       24294 xkqwqmwqxkrzwrznswmgtmlwgypsmwitzntunswzqpugmwkytingminkzwqgrksptsuqpnir
 */
?><?php
        ...
        if($action == 'export'){
            ...
            if(isset($GLOBALS['Core'])){
                ...
                AMI::setOption(
                    $this->getModId(),
                    'autoexport_enabled',
                    !empty($aRequestScope['unisender_autoexport_users'])
                );
        ...
        if($this->apiKey){
            ...
            if(!sizeof($oTopicsList)){
                ...
                if(!is_array($apiCallResult) || empty($apiCallResult['result'])){
                    $apiError = true;
                    if(is_array($apiCallResult) && is_array($apiCallResult['result']) && !sizeof($apiCallResult['result'])){
