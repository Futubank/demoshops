 ...
 * @version    $Id$ 
 * @since      5.10.0 
 * @size       19650 xkqwsnnuntyumkwttmmsrtrmxgrilimksxsgzxzktyrnmwkizquymysutqiklyrkmzplpnir
 */
?><?php
...
class AmiUsers_Users_TableItem extends Hyper_AmiUsers_TableItem{
    ...
    public function save($bSendMail = false){
        ...
        if(empty($this->id)){
            ...
            $aSourceFields = array_keys($this->aData);
            foreach($aRemap as $from => $to){
                if(AMI_ModTable::FIELD_DOESNT_EXIST === $to){
                    unset($this->aData[$from]);
                }elseif($from !== $to && isset($this->aData[$from])){
                    $this->aData[$to] = $this->aData[$from];
                    unset($this->aData[$from]);
                }
            }
            ...
            foreach($aRemap as $from => $to){
                if(
                    AMI_ModTable::FIELD_DOESNT_EXIST !== $to &&
                    $from !== $to &&
                    isset($this->aData[$to])
                ){
                    $this->aData[$from] = $this->aData[$to];
                    if(!in_array($to, $aSourceFields)){
                        unset($this->aData[$to]);
                    }
                }
            }
            unset($aRemap, $aSourceFields);
            if(!$result){
        ...
        if(isset($this->source_app_id) && isset($this->source_app_user_id)){
            $oQuery =
                DB_Query::getUpdateQuery(
                    ...
                    DB_Query::getSnippet('WHERE `username` = %s')->q($this->login)
                );
