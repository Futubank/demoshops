 ...
 * @version    $Id$ 
 * @since      6.0.2 
 * @size       44108 xkqwlgksiwpzzstpztxpsnnwstyrlqukqwnguqtsglwxmswswwyqnugkgqqsulitwnyqpnir
 */
?><?php
...
// Call autoload
class_exists('Hyper_AmiAsync_Frn');

/**
 *
 */
class Hyper_AmiMultifeeds_ListFrn extends Hyper_AmiAsync_ListFrn{
}

/**
 *
 */
class Hyper_AmiMultifeeds_ListViewFrn extends Hyper_AmiAsync_ListViewFrn{
}

/**
 *
 */
class Hyper_AmiMultifeeds_FormFrn extends Hyper_AmiAsync_FormFrn{
}

/**
 *
 */
class Hyper_AmiMultifeeds_FormViewFrn extends AMI_Module_FormViewAdm{
    /**
     ...
     * @amidev Temporary
     */
    protected $aFormButtons = array('cancel', 'add', 'save');
    ...
    protected $aPlaceholders = array(
        '#form',
            ...
            '#default_tabset',
                ...
                'options_tab',
            'default_tabset',
            'public', 'id_dataset', 'id_page', 'id_cat', 'ext_custom_fields_addon', 'date_created', 'ext_adv', 'ext_image', 'ext_tags', 'ext_relations',
            '#ext_custom_fields_bottom', 'ext_custom_fields_bottom',
        'form'
    );

    /**
     * Array of form buttons to exclude on form mode (new, edit).
     *
     * @var    array
     * @amidev Temporary
     */
    protected $aExcludedButtons = array(
        'new'  => array('save'),
        'edit' => array('add')
    );
    ...
    public function init(){
        parent::init();
        if(strpos($this->getModId(), '_cat') !== FALSE){
            $this->dropField('date_created');
        }
        $this->dropField('og_image');
        $this->dropField('hide_in_list');
        $this->dropField('sticky');
        $this->dropField('date_sticky_till');
        $this->dropField('sublink');
        $this->addField(array('name' => 'sublink', 'validate' => array('sublink')));
    ...
    protected function setFilter(){
        ...
        if($aBrowser['useSpecView']){
            $this->oList->addWhereDef(
                DB_Query::getSnippet(
                    'AND (`i`.`' . $this->oModel->getFieldName('hide_in_list') . '` = 0' .
                    AMI_GlobalFilters::addRightsFilterAddon($this->getModId(), 'i') . ')'
                )
            );
        }
        ...
        switch($this->specblockMode){
            case 'no_cats':
                ...
                if($aBrowserData['useSpecView']){
                    ...
                    if(AMI::issetAndTrueOption($modId, 'hide_future_items')){
                        $snippet .= " AND " . $this->oModel->getFieldName('date_created') . ' <= NOW()';
                    }
                }
            ...
            case 'cats_num':
                ...
                if(sizeof($this->aCatIds) > 0){
                    ...
                    $commonFilter = ' AND `i`.`lang` = %s AND `i`.`public` = 1';
                    if(AMI::issetAndTrueOption($modId, 'hide_future_items')){
                        $commonFilter .= " AND `i`." . $this->oModel->getFieldName('date_created') . ' <= NOW()';
                    }
...
class Hyper_AmiMultifeeds_BrowseItemsViewFrn extends AMI_ModItemsView{
    ...
    public function dispatchView($name, array $aEvent, $handlerModId, $srcModId){
        ...
        AMI_Registry::set('page/itemHeader', $aEvent['aData']['header']);
 ...
 *
 */
class Hyper_AmiMultifeeds_CatListViewFrn extends Hyper_AmiAsync_ListViewFrn{
    /**
     * Order column
     *
     * @var string
