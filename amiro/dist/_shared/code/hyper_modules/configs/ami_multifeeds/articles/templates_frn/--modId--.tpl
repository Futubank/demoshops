...
Шаблоны модуля подключаются и наследуются в следующем порядке:
  ...
  2. Шаблон конфигурации "Статьи" содержит и переопределяет только элементы оформления, которые должны
     отличаться от базового модуля и являются специфичными для модулей этой конфигурации. /hyper/ami_multifeeds_news.tpl
...
Templates are including and are inherited in the following order:
  ...
  2. Template of configuration "Articles" contains and overrides only design elements that must
     are different from the base module and are specific to the modules of this configuration. /hyper/ami_multifeeds_news.tpl
...
##--
Подключаемый шаблон кофигурации "Статьи" "ami_multifeeds_articles.tpl"

Included template of configuration "Articles" "ami_multifeeds_articles.tpl"
--##
%%include_template "templates/hyper/ami_multifeeds_articles.tpl"%%
