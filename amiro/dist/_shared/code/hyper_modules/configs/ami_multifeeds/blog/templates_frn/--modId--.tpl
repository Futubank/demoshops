...
Шаблоны модуля подключаются и наследуются в следующем порядке:
  ...
  2. Шаблон конфигурации "Блог" содержит и переопределяет только элементы оформления, которые должны
...
Templates are including and are inherited in the following order:
  ...
  2. Template of configuration "Blog" contains and overrides only design elements that must
     are different from the base module and are specific to the modules of this configuration. /hyper/ami_multifeeds_blog.tpl
...
##--
Подключаемый шаблон кофигурации "Блог" "ami_multifeeds_blog.tpl"

Included template of configuration "Blog" "ami_multifeeds_blog.tpl"
--##
%%include_template "templates/hyper/ami_multifeeds_blog.tpl"%%
