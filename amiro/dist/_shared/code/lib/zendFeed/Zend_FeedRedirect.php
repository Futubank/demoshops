 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_FeedRedirect
{
	protected static $maxRedirects = 5;
    ...
    public static function importString($string)
    {
        ...
        if ($doc->getElementsByTagName('redirect')->item(0)
        	&& $doc->getElementsByTagName('newLocation')->item(0))
        {
        	if (self::$redirects < self::$maxRedirects)
        	{
        		$location = $doc->getElementsByTagName('newLocation')->item(0)->nodeValue;
        		++self::$redirects;
	        	//return a newly created Zend_Feed_Rss object
    	        return self::import($location);
        	}
        	else
        	{
        		throw new Zend_Feed_Exception('XML redirects limit exceeded');
        	}
     ...
     * @return Zend_Feed_Abstract
     */
    public static function importArray($data, $format = 'atom')
    {
    	if( !is_array($data))
    	{
    		throw new Zend_Exception('[data] must be array');
    	}
