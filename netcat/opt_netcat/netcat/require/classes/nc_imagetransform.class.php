...
class nc_ImageTransform extends nc_System {
    ...
    public static function getBestFitDimensions($original_width, $original_height, $resize_width, $resize_height) {
        if (!$original_width && !$original_height) {
            return array($original_width, $original_height);
        }

        $x_ratio = $resize_width / $original_width;
        $y_ratio = $resize_height / $original_height;

        if (!$resize_width) {
            return array(floor($y_ratio * $original_width), $resize_height);
        }

        if (!$resize_height) {
            return array($resize_width, floor($x_ratio * $original_height));
        }
    ...
    public static function imgResize($src_img, $dest_img, $width, $height, $mode = 0, $format = NULL, $quality = 90, $message_id = 0, $field = 0, $color = "ffffff") {
        ...
        if ($mode == 0) {
            list($new_width, $new_height) = self::getBestFitDimensions($img_size[0], $img_size[1], $width, $height);
        } // proportionality
        elseif ($mode == 1) {
            $new_height = $height;
            $new_width = $width;
