<?php
	...
	class umiInstallExecutor {
		...
		private function saveOverwritableState() {
			...
			$svId = umiObjectsCollection::getInstance()
				->getObjectIdByGUID('system-supervisor');
			if (!is_numeric($svId) || $svId == 0) {
				$svId = 14;
			}

			/** @noinspection PhpUndefinedMethodInspection (метода нет в интерфейсе; вызов оставлен для обратной совместимости) */
			$typeId = umiObjectTypesCollection::getInstance()->getBaseType('users', 'user');
			$properties = umiPropertiesHelper::getInstance();
			$noCache = true;
			...
			$state = [
				'login' => $properties->getPropertyValue($svId, 'login', $typeId, $noCache),
				'md5pass' => $properties->getPropertyValue($svId, 'password', $typeId, $noCache),
				'email' => $properties->getPropertyValue($svId, 'e-mail', $typeId, $noCache),
				'fname' => $properties->getPropertyValue($svId, 'fname', $typeId, $noCache),
				'lname' => $properties->getPropertyValue($svId, 'lname', $typeId, $noCache),
				'mname' => $properties->getPropertyValue($svId, 'father_name', $typeId, $noCache),
